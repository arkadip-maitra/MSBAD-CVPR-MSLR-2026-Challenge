#!/usr/bin/env python3
"""
v44: ConvNeXt V1 Tiny @ 384×384 + SWA
Genuinely new architecture: ConvNeXt V1 (no GRN layers) vs V2 (with GRN).
Same channel dims [96,192,384,768] — partial born-again from v18 (ConvNeXtV2@384)
with strict=False: 160/178 params transfer, 18 LayerScale params stay ImageNet.
KD from vit_v9 (cross-architecture teacher for diversity).
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v44")
...
class MultiScaleConvNeXt(nn.Module):
    """Works for both ConvNeXt V1 and V2 since they share the same feature structure."""
...
def load_born_again_partial(model, cp, dev):
    """Load born-again checkpoint with strict=False for V2→V1 transfer."""
    ...
    log.info("  Partial born-again ← %s (val %.2f%%)", cp.name, 100*ck.get("val_acc",0))
    log.info("    Transferred: %d params, Missing (V1-only): %d, Unexpected (V2-only): %d",
             len(model.state_dict()) - len(missing), len(missing), len(unexpected))
...
def get_llrd_params(model, base_lr, decay=0.75, wd=0.05):
    ...
    for name,p in model.named_parameters():
        ...
        elif "backbone.stages_0" in name or "backbone.stages.0" in name: lr=base_lr*decay**4
        elif "backbone.stages_1" in name or "backbone.stages.1" in name: lr=base_lr*decay**3
        elif "backbone.stages_2" in name or "backbone.stages.2" in name: lr=base_lr*decay**2
        elif "backbone.stages_3" in name or "backbone.stages.3" in name: lr=base_lr*decay**1
        else: lr=base_lr
...
def main():
    ...
    log.info("★★★ ConvNeXt V1 (no GRN) @ %d×%d — NEW ARCHITECTURE ★★★", CFG["img_size"], CFG["img_size"])
    ...
    log.info("KD α=%.2f τ=%.1f | Teacher: vit_v9 (cross-arch) | Born-again: v18 (V2→V1 partial)",alpha,tau)
    ...
    for fold,(tri,vai) in enumerate(skf.split(keys,labels)):
        ...
        model=MultiScaleConvNeXt(CFG["model_name"],CFG["n_classes"],pretrained=True,
                                 drop_path_rate=CFG["drop_path_rate"],proj_dim=CFG["proj_dim"],
                                 drop_rate=CFG["drop_rate"],ms_samples=CFG["ms_dropout_samples"]).to(dev)
        ba=V18_DIR/f"best_fold{fold}.pt"
        if ba.exists():
            load_born_again_partial(model, ba, dev)
    ...
    fms=[]
    for f in range(CFG["n_folds"]):
        m=MultiScaleConvNeXt(CFG["model_name"],CFG["n_classes"],pretrained=False,drop_path_rate=0.0,
