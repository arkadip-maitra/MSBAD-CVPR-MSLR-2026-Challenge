...
ViT-Tiny (augreg, ImageNet-21k pretrained) with LLRD, heavy augmentation, TTA.

Key differences vs. CNN scripts (train.py / train_v2.py):
  - ViT-Tiny backbone (5.5M params) via timm, input (3, 64, 256), 64 tokens
  - Layer-wise learning rate decay (LLRD, factor 0.65)
  - Lower peak LR (2e-4), higher weight decay (0.05)
  - Higher Mixup alpha (0.8), random erasing (0.25)
  - Longer warmup (10% of training)

Usage:
    conda run -n slr python /data/slr/train_vit.py
"""
...
# Logging
# ---------------------------------------------------------------------------
OUT_DIR = Path("/data/slr/checkpoints_vit")
...
CFG = {
    ...
    # Data shape — pad time to 64 for ViT (divisible by patch_size=16)
    "max_frames": 64,
    "n_range_bins": 256,
    "n_classes": 126,
    # Global z-score stats (same as CNN scripts)
    "channel_means": [-64.3217, -62.9119, -63.9254],
    "channel_stds": [12.5721, 11.6620, 11.7259],
    # Training — ViT-specific recipe
    "n_folds": 5,
    "batch_size": 64,
    "num_workers": 4,
    "epochs": 120,
    ...
    # LLRD
    "llrd_decay": 0.65,
    # Augmentation — stronger than CNN
    "mixup_alpha": 0.8,
    "cutmix_alpha": 1.0,
    "cutmix_prob": 0.5,
    "time_mask_max": 8,
...
# ===========================================================================
#  DATASET (adapted for ViT: max_frames=64, random erasing)
# ===========================================================================
...
# ===========================================================================
#  MODEL: ViT-Tiny via timm
# ===========================================================================


def create_vit(n_cls=126, pretrained=True):
...
def get_llrd_param_groups(model, base_lr, decay, wd):
    """Assign progressively lower LR to earlier transformer blocks.

    Groups (from lowest to highest LR):
      - patch_embed, cls_token, pos_embed  →  lr * decay^(depth+1)
      - blocks.0                           →  lr * decay^depth
      - blocks.1                           →  lr * decay^(depth-1)
      - ...
      - blocks.(depth-1)                   →  lr * decay^1
      - norm, head                         →  lr * 1.0
    """
    ...
    for name, param in model.named_parameters():
        ...
        scale = decay ** (num_blocks + 1 - layer_id)
        lr = base_lr * scale
    ...
    log.info(
        "LLRD: %d param groups, LR range [%.2e … %.2e]",
        len(param_groups),
...
# ===========================================================================
#  TEST-TIME AUGMENTATION (adapted for 64×256 ViT input)
# ===========================================================================


def _tta_transforms(x: torch.Tensor) -> list[torch.Tensor]:
    """6 augmented views for TTA on (B, 3, 64, 256) input."""
    views = [x]
    views.append(x.flip(dims=[2]))                    # time-reversed
    views.append(x.roll(-4, dims=2))                  # shift left 4 frames
    views.append(x.roll(4, dims=2))                   # shift right 4 frames
    views.append(x[:, [1, 2, 0], :, :])               # channel permutation
    views.append(x + 0.05 * torch.randn_like(x))      # slight noise
    return views
...
def main():
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(keys, labels)):
        ...
        # ---- model --------------------------------------------------------
        model = create_vit(
            n_cls=CFG["n_classes"], pretrained=CFG["pretrained"]
        ).to(dev)
        ...
        param_groups = get_llrd_param_groups(
            model, CFG["lr"], CFG["llrd_decay"], CFG["weight_decay"]
        )
    ...
    for fold in range(CFG["n_folds"]):
        m = create_vit(n_cls=CFG["n_classes"], pretrained=False).to(dev)
        ckpt = torch.load(
    ...
    # Without TTA
    probs_no_tta = predict_ensemble_tta(
        fold_models, test_loader, dev, use_tta=False
    )
    ...
    probs_tta = predict_ensemble_tta(
        fold_models, test_loader, dev, use_tta=True
    )
