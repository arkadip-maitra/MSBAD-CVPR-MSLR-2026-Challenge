...
CNN v12: EfficientNetV2-S (IN-22k) + Multi-Scale Feature Aggregation
         + cross-architecture KD from v11 (ConvNeXtV2) & vit_v9 (CAFormer)

Totally different backbone: MBConv + Squeeze-and-Excitation blocks.
"""
...
OUT_DIR = Path("/data/slr/checkpoints_v12")
...
# ── student: EfficientNetV2-S with multi-scale aggregation ───────────────────
...
# ── teacher models (for loading v11 & vit_v9 checkpoints) ────────────────────
...
PL_CONFIGS = TEACHER_CONFIGS


# ── LLRD for EfficientNetV2 ──────────────────────────────────────────────────
...
def generate_pseudo_labels(eval_keys, eval_cache, pl_configs, dev,
                           threshold=0.55):
    log.info("Generating pseudo-labels (threshold=%.2f) …", threshold)
    ds = RadarDataset(eval_keys, [0] * len(eval_keys), eval_cache,
    ...
    log.info("  Loaded %d teacher models for PL", len(teachers))
    ...
    pl_soft = {eval_keys[i]: probs[i]
               for i in range(len(eval_keys)) if mask[i]}
    n_kept = len(pl_keys)
...
def precompute_teacher_labels(keys, cache, teacher_configs, dev):
    ...
    log.info("  Loaded %d teacher models for KD", len(teachers))
...
def train_epoch(model, loader, optim, sched, crit, dev, alpha, tau,
    ...
    for x, y, soft in tqdm(loader, desc="  train", leave=False):
        ...
        kd = lam * kd_loss_fn(logits, sa, tau) + \
...
def main():
    ...
    soft_labels = precompute_teacher_labels(keys, train_cache,
                                            TEACHER_CONFIGS, dev)
    ...
    log.info("Loading v12 checkpoints for ensemble …")
    models_ens = []
    for fold in range(CFG["n_folds"]):
    ...
    probs_no = predict_ensemble_tta(models_ens, eval_loader, dev, use_tta=False)
    ...
    probs_tta = predict_ensemble_tta(models_ens, eval_loader, dev, use_tta=True)
    ...
    log.info("Saved v12 TTA probs → %s", out / "eval_probs_tta.pt")
