#!/usr/bin/env python3
"""
v21: ConvNeXtV2-Tiny with LOG-SCALE + PER-SAMPLE normalization
BREAKTHROUGH ATTEMPT: All previous models used global z-score normalization.
This means they all see identical pixel distributions → correlated errors.
By using log-scale + per-sample normalization, the model sees FUNDAMENTALLY
different features: high-variance regions (gestures) get compressed,
low-variance regions get expanded. This creates truly uncorrelated predictions.

Trains at BOTH 224 and 384, generates predictions for each.
Uses ImageNet pretrained weights (not born-again — preprocessing is too different).
KD from v11+vit_v9 soft labels (class probs are preprocessing-independent).
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v21")
...
_MEANS_ZSCORE = np.array(CFG["channel_means"],dtype=np.float32).reshape(3,1,1)
_STDS_ZSCORE = np.array(CFG["channel_stds"],dtype=np.float32).reshape(3,1,1)
_ERASER=RandomErasing(p=CFG["random_erase_prob"],scale=(0.02,0.2),ratio=(0.3,3.3),value=0)

class RadarDatasetLogScale(Dataset):
    """Uses log-scale + per-sample normalization instead of global z-score."""
    ...
    def _log_normalize(x):
        """Log-scale + per-sample standardization."""
        x_min = x.min()
        x = x - x_min + 1.0
        x = np.log(x)
        mu = x.mean()
...
class RadarDatasetZScore(Dataset):
    """Standard z-score for teacher inference only."""
    def __init__(self, keys, labels, cache, augment=False, soft_labels=None,
                 img_size=224, max_frames=48):
        self.keys, self.labels, self.cache = keys, labels, cache
        self.augment, self.soft_labels = augment, soft_labels
    ...
    def __getitem__(self, idx):
        x=self.cache[self.keys[idx]].copy()
        x=(x-_MEANS_ZSCORE)/_STDS_ZSCORE
        x=self._pad_or_crop(x, self.max_frames, False)
        t=torch.from_numpy(x)
        sz=(self.img_size, self.img_size) if isinstance(self.img_size, int) else self.img_size
        t=F.interpolate(t.unsqueeze(0),size=sz,mode="bilinear",align_corners=False).squeeze(0)
...
def precompute_teacher_labels(keys, cache, configs, dev):
    ...
    ds=RadarDatasetZScore(keys,[0]*len(keys),cache,augment=False,
...
def main():
    ...
    log.info("★★★ LOG-SCALE PREPROCESSING (not z-score!) ★★★")
    ...
    log.info("KD α=%.2f τ=%.1f | LOG-SCALE preprocessing | NO born-again (fresh ImageNet)",alpha,tau)
    ...
    for fold,(tri,vai) in enumerate(skf.split(keys,labels)):
        ...
        tds=RadarDatasetLogScale(trk,trl,train_cache,augment=True,soft_labels=soft_labels,
                                 img_size=CFG["img_size"],max_frames=CFG["max_frames"])
        vds=RadarDatasetLogScale(vak,val_,train_cache,augment=False,
                                 img_size=CFG["img_size"],max_frames=CFG["max_frames"])
        ...
        log.info("  Using fresh ImageNet pretrained weights (no born-again)")
        crit=nn.CrossEntropyLoss(label_smoothing=CFG["label_smoothing"])
        best_acc=0.0; ckpt=out/f"best_fold{fold}.pt"

        # Stage 1: head warmup (longer since no born-again)
    ...
    eds=RadarDatasetLogScale(eval_keys,[0]*len(eval_keys),eval_cache,augment=False,
                             img_size=CFG["img_size"],max_frames=CFG["max_frames"])
