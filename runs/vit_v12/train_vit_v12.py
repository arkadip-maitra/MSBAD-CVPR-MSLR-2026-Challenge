#!/usr/bin/env python3
"""
vit_v12: CAFormer-S18 + Supervised Contrastive Loss + SAM
Born-again from vit_v9, NO pseudo-labels, cross-arch KD from v11 + v12
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_vit_v12")
...
CFG = {
    ...
    "n_folds": 5, "batch_size": 80, "num_workers": 4, "img_size": 224,
    "stage1_epochs": 5, "stage1_lr": 2e-4,
    "stage2_epochs": 100, "stage2_lr": 1.5e-5,
...
def _build_ms(bb,pd,nc,dr,ms,pod):
    sd=bb.feature_info.channels()
    g=nn.ModuleList([GeM(3.0) for _ in sd])
...
class MultiScaleCAFormer(nn.Module):
    def __init__(s,mn,nc=126,pt=True,dpr=0.15,pd=384,dr=0.3,ms=5,pod=0):
...
class MultiScaleEfficientNetV2(nn.Module):
    def __init__(s,mn,nc=126,pt=True,dpr=0.0,pd=384,dr=0.3,ms=5,pod=0):
        super().__init__(); s.backbone=timm.create_model(mn,pretrained=pt,features_only=True,out_indices=(0,1,2,3,4),drop_path_rate=dpr)
        s.stage_gems,s.stage_projs,s.scale_attn,s.head,s.proj_head=_build_ms(s.backbone,pd,nc,dr,ms,pod)
    def forward(s,x,return_proj=False): return _fwd_ms(s.backbone,s.stage_gems,s.stage_projs,s.scale_attn,s.head,s.proj_head,x,return_proj)
...
TEACHER_CONFIGS = [
    ...
    {"dir": V12_DIR, "cls": MultiScaleEfficientNetV2, "model_name": "tf_efficientnetv2_s.in21k_ft_in1k",
     "kwargs": {"dpr":0.0,"pd":384,"dr":0.3,"ms":5}},
]

def load_born_again(model,cp,dev):
    ck=torch.load(cp,map_location=dev,weights_only=True)
    mi,_=model.load_state_dict(ck["model_state_dict"],strict=False)
    log.info("  Born-again ← %s (val %.2f%%, new_keys=%d)",cp.name,100*ck.get("val_acc",0),len(mi))
...
def predict_tta(ms,loader,dev,tta=True):
    ...
    for x,_,_ in tqdm(loader,desc="  predict",leave=False):
        ...
        for v in (_tta(x) if tta else [x]):
            for m in ms: bp+=F.softmax(m(v),1)
        ap.append((bp/(len(_tta(x) if tta else [x])*len(ms))).cpu())
    return torch.cat(ap)
...
def main():
    ...
    log.info("Loss: %.1f*CE + %.1f*SupCon + %.1f*KD | SAM | NO PL",CFG["ce_weight"],CFG["supcon_weight"],CFG["kd_weight"])
    ...
    for fold,(tri,vai) in enumerate(skf.split(keys,labels)):
        ...
        model=MultiScaleCAFormer(CFG["model_name"],nc=CFG["n_classes"],pt=True,
        ...
        log.info("── Stage 1: head+proj warmup ──")
        ...
        log.info("── Stage 2: SAM+LLRD+SupCon ──")
        ...
        for ep in range(1,CFG["stage2_epochs"]+1):
            ...
            if va_>best_acc:
                best_acc=va_; pat=0; torch.save({"fold":fold,"epoch":ep,"stage":2,"model_state_dict":model.state_dict(),"val_acc":va_},ckpt)
                log.info("  ★ best %.2f%%",100*va_)
            else:
                pat+=1
    ...
    fms=[]
    for f in range(CFG["n_folds"]):
        m=MultiScaleCAFormer(CFG["model_name"],nc=CFG["n_classes"],pt=False,dpr=0.0,pd=CFG["proj_dim"],dr=CFG["drop_rate"],ms=CFG["ms_dropout_samples"],pod=CFG["proj_out_dim"]).to(dev)
