#!/usr/bin/env python3
"""v31: CAFormer-S18 SEED=2024 @ 224×224 + SWA + KD — born-again from vit_v9, different seed for diversity."""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v31")
...
CFG = {
    ...
    "stage2_epochs": 80, "stage2_lr": 5e-5,
...
def main():
    ...
    log.info("★★★ CAFormer-S18 SEED=%d @ %d ★★★",CFG["seed"],IMG_SIZE)
    ...
    for fold,(tri,vai) in enumerate(skf.split(keys,labels)):
        ...
        model=MultiScaleCAFormer(MODEL_NAME,CFG["n_classes"],pretrained=True,
    ...
    fms=[]
    for f in range(CFG["n_folds"]):
        m=MultiScaleCAFormer(MODEL_NAME,CFG["n_classes"],pretrained=False,drop_path_rate=0.0,
