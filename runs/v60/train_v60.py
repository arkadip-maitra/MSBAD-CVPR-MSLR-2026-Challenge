#!/usr/bin/env python3
"""
v60: ConvNeXtV2-Base @ 224 (88M params) — single fold, extreme 8-teacher KD, heavy regularization.
No born-again (dims differ from Tiny). Relies on strong KD to bridge the domain gap.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v60")
...
MultiScaleConvNeXtV2 = MultiScaleModel
...
def get_llrd_params(model, base_lr, decay=0.90, wd=0.08):
    ...
    for name,p in model.named_parameters():
        ...
        depth = None
        if "backbone" in name:
            for tag in ["stem", "stages_0", "stages.0"]:
                if tag in name: depth = 5; break
            if depth is None:
                for tag in ["stages_1", "stages.1"]:
                    if tag in name: depth = 4; break
            if depth is None:
                for tag in ["stages_2", "stages.2"]:
                    if tag in name: depth = 3; break
            if depth is None:
                for tag in ["stages_3", "stages.3"]:
                    if tag in name: depth = 2; break
            if depth is None:
                for tag in ["norm", "downsample"]:
                    if tag in name: depth = 3; break
            if depth is None:
                depth = 3
...
def main():
    ...
    log.info("Student: ConvNeXtV2-Base@224 (88M) — SINGLE FOLD %d", CFG["run_fold"])
    ...
    log.info("KD α=%.2f τ=%.1f | SWA %d ep | SINGLE FOLD | 8-teacher KD | AMP",alpha,tau,CFG["swa_epochs"])
