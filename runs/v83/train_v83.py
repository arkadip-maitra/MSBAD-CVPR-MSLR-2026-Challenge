#!/usr/bin/env python3
"""
v83: DaViT-Tiny — Dual Attention Vision Transformer for architecture diversity.
...
OUT_DIR = Path("/data/slr/checkpoints_v83")
...
def load_born_again_head_only(model, cp, dev):
    """Load only the head weights from a ConvNeXtV2 checkpoint (same stage dims [96,192,384,768])."""
    ...
    log.info("  Partial born-again: loaded %d head params from %s (val %.2f%%)",
             loaded, cp.name, 100*ck.get("val_acc",0))

def get_llrd_params(model, base_lr, decay=0.75, wd=0.05):
    groups={}
    for name,p in model.named_parameters():
        if not p.requires_grad: continue
        if "backbone.patch_embed" in name or "backbone.stem" in name:
            lr=base_lr*decay**5
        elif "backbone.stages_0" in name or "backbone.stages.0" in name:
            lr=base_lr*decay**4
        elif "backbone.stages_1" in name or "backbone.stages.1" in name:
            lr=base_lr*decay**3
        elif "backbone.stages_2" in name or "backbone.stages.2" in name:
            lr=base_lr*decay**2
        elif "backbone.stages_3" in name or "backbone.stages.3" in name:
            lr=base_lr*decay**1
        else: lr=base_lr
...
def main():
    ...
    log.info("DaViT-Tiny | partial born-again head from v66 | Fold %d", TARGET_FOLD)
    ...
    log.info("="*64); log.info("FOLD %d — DaViT-Tiny backbone",TARGET_FOLD); log.info("="*64)
