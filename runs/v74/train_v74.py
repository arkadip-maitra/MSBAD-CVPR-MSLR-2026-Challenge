#!/usr/bin/env python3
"""
v74: 6-channel input — 3 RTM + 3 frame-by-frame temporal differences.
Frame diffs approximate velocity/Doppler which is key for gesture recognition.
Different from v63's mean subtraction (MTI) — this captures instantaneous motion.
Born-again from v66 with stem expansion from 3→6 channels.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v74")
...
def collect_eval_samples(dd):
    ...
    return s
def _load_one(sp,sn):
    rtms = np.stack([np.load(os.path.join(sp,f"{sn}_RTM{r}.npy")) for r in (1,2,3)],0).astype(np.float32)
    return rtms
...
class RadarDataset(Dataset):
    ...
    def _apply_aug_6ch(self, x):
        ...
        if np.random.random()<CFG["channel_shuffle_prob"]:
            ...
            x=np.concatenate([x[perm], x[perm+3]], axis=0)
    ...
    def __getitem__(self, idx):
        ...
        x_norm = (x - _MEANS) / _STDS
        d_norm = diff / _DIFF_STD
        x6 = np.concatenate([x_norm, d_norm], axis=0)
        x6 = self._pad_or_crop(x6, self.max_frames, self.augment)
        if self.augment: x6 = self._apply_aug_6ch(x6)
        t = torch.from_numpy(x6.copy())
...
class MultiScaleConvNeXtV2(nn.Module):
    def __init__(self, model_name, n_classes=126, pretrained=True,
                 drop_path_rate=0.2, proj_dim=512, drop_rate=0.3, ms_samples=5, in_chans=6):
        super().__init__()
...
def expand_stem_to_6ch(model, ckpt_sd):
    """Expand 3-channel stem to 6 channels by duplicating weights."""
    stem_key = None
    for k in ckpt_sd:
        if 'stem_0.weight' in k or ('stem' in k and 'weight' in k and ckpt_sd[k].shape[1] == 3):
            stem_key = k
            break
    if stem_key is None:
        log.warning("No 3-channel stem found to expand")
        return
    w3 = ckpt_sd[stem_key]
    ckpt_sd[stem_key] = torch.cat([w3, w3], dim=1) * 0.5
    log.info("Expanded stem %s from 3→6 channels", stem_key)

def load_born_again(model, cp, dev):
    ck=torch.load(cp,map_location=dev,weights_only=True)
    sd = ck["model_state_dict"]
    expand_stem_to_6ch(model, sd)
...
def _tta(x):
    p1=torch.cat([x[:,[1,2,0]],x[:,[4,5,3]]],1)
    p2=torch.cat([x[:,[2,0,1]],x[:,[5,3,4]]],1)
    return [x,x.flip(2),x.roll(-4,2),x.roll(4,2),p1,p2,
...
def main():
    ...
    log.info("6CH RTM+DIFF | Born-again v66 stem-expand | Fold %d", TARGET_FOLD)
    ...
    log.info("="*64); log.info("FOLD %d — 6CH RTM+DIFF, born-again v66",TARGET_FOLD); log.info("="*64)
