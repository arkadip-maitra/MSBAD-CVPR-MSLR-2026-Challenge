#!/usr/bin/env python3
"""
v72: Fix short-sequence bottleneck — reflection padding instead of tiling.
Error analysis showed 16-20 frame samples have 72.78% vs 97.06% for 35+ frames.
Tiling repeats the gesture 2-3x creating misleading periodicity artifacts.
Reflection padding preserves temporal structure naturally.
Born-again from v66.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v72")
...
class RadarDataset(Dataset):
    ...
    def _pad_or_crop(x, max_t, random_crop=False):
        ...
        if t>max_t:
            ...
            x = np.pad(x, ((0,0),(pad_left,pad_right),(0,0)), mode='reflect')
        return x
...
def main():
    ...
    log.info("REFLECTION PAD (no tiling) | Born-again v66 | Fold %d", TARGET_FOLD)
    ...
    log.info("="*64); log.info("FOLD %d — REFLECTION PAD, born-again v66",TARGET_FOLD); log.info("="*64)
