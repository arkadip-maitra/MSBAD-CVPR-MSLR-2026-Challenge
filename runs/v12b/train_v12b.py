#!/usr/bin/env python3
"""
v12b: EfficientNetV2-S born-again from v12 + triple cross-arch KD (ConvNeXtV2 + CAFormer teachers)
"""
...
OUT_DIR = Path("/data/slr/checkpoints_v12b")
...
def main():
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(keys, labels)):
        ...
        ckpt_path = V12_DIR / f"best_fold{fold}.pt"
        if ckpt_path.exists():
            load_born_again(model, ckpt_path, dev)
    ...
    log.info("Loading v12b checkpoints for ensemble …")
    models_ens = []
    for fold in range(CFG["n_folds"]):
    ...
    log.info("Saved v12b TTA probs → %s", out / "eval_probs_tta.pt")
