#!/usr/bin/env python3
"""
v111: 5-fold CV — ConvNeXtV2-Tiny born-again from v106 + PSEUDO-LABELING.
High-confidence test samples (all 5 folds agree) added to training with soft labels.
Trained with combined hard-label CE + soft-label KD loss. WEAK augmentation.
Usage: python train_5fold_v111.py --fold 0 --gpu 0
"""
...
VERSION = "v111"
PARENT_DIR = Path("/data/slr/checkpoints_v106")
PSEUDO_DIR = Path("/data/slr/checkpoints_v106")
OUT_DIR = Path(f"/data/slr/checkpoints_{VERSION}")
OUT_DIR.mkdir(parents=True, exist_ok=True)
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s",
                    handlers=[logging.FileHandler(OUT_DIR/f"training_fold{args.fold}.log"), logging.StreamHandler()])
...
CFG = {
    ...
    "n_folds": 5, "batch_size": 48, "num_workers": 2,
    "stage1_epochs": 3, "stage1_lr": 1e-4,
    "stage2_epochs": 400, "stage2_lr": 5e-5, "pseudo_conf_thresh": 0.75,
    "kd_temperature": 3.0, "kd_alpha": 0.3,
    "swa_epochs": 30, "swa_lr": 3e-6,
    "weight_decay": 0.05, "warmup_pct": 0.05,
    "label_smoothing": 0.1,
    "grad_clip_norm": 1.0, "llrd_decay": 0.75,
    "mixup_alpha": 0.2, "cutmix_alpha": 0.5, "cutmix_prob": 0.3,
    "time_mask_max": 4, "range_mask_max": 15, "noise_std": 0.05,
    "time_warp_lo": 0.9, "time_warp_hi": 1.1,
    "time_reverse_prob": 0.3, "channel_shuffle_prob": 0.0, "channel_drop_prob": 0.0,
    "random_erase_prob": 0.10, "circ_shift_prob": 0.2, "circ_shift_max": 4,
    "amp_scale_prob": 0.3, "amp_scale_lo": 0.9, "amp_scale_hi": 1.1,
    "model_name": "convnextv2_tiny.fcmae_ft_in22k_in1k",
    "drop_path_rate": 0.1, "proj_dim": 512, "drop_rate": 0.2,
...
_ERASER=RandomErasing(p=CFG["random_erase_prob"],scale=(0.02,0.15),ratio=(0.3,3.3),value=0)
...
class RadarDataset(Dataset):
    ...
    def _apply_aug(self, x):
        ...
        if np.random.random()<0.3: x=x+np.random.normal(0,CFG["noise_std"],x.shape).astype(np.float32)
...
class PseudoRadarDataset(Dataset):
    """Dataset for pseudo-labeled samples with soft labels."""
    def __init__(self, keys, soft_labels, cache, augment=False, img_size=224, max_frames=48):
        self.keys, self.soft_labels, self.cache = keys, soft_labels, cache
        self.augment = augment
        self.img_size, self.max_frames = img_size, max_frames
    def __len__(self): return len(self.keys)
    @staticmethod
...
class MultiSampleDropout(nn.Module):
    def __init__(self,inf,outf,n_samples=5,drop_rate=0.2):
...
class MultiScaleConvNeXtV2(nn.Module):
    def __init__(self, model_name, n_classes=126, pretrained=True,
                 drop_path_rate=0.1, proj_dim=512, drop_rate=0.2, ms_samples=5):
        super().__init__()
...
def train_epoch(model, loader, opt, sched, crit, dev, scaler, use_mix=True):
    ...
    for x,y in tqdm(loader,desc="  train",leave=False):
        ...
        if use_mix:
            ...
            if np.random.random()<CFG["cutmix_prob"]:
                ...
                xm=x.clone(); xm[:,:,t0:t1,r0:r1]=x[idx,:,t0:t1,r0:r1]; lam=1.0-(t1-t0)*(r1-r0)/(x.size(2)*x.size(3))
            else:
                lam=float(np.random.beta(CFG["mixup_alpha"],CFG["mixup_alpha"])); xm=lam*x+(1-lam)*x[idx]
            ya,yb=y,y[idx]
        else: xm,ya,yb,lam=x,y,y,1.0
        with torch.amp.autocast('cuda'):
            logits=model(xm); loss=lam*crit(logits,ya)+(1-lam)*crit(logits,yb)
        ...
        co+=(lam*logits.argmax(1).eq(ya).float()+(1-lam)*logits.argmax(1).eq(yb).float()).sum().item(); n+=bs
    return tl/n,co/n

def train_epoch_kd(model, real_loader, pseudo_loader, opt, sched, crit, dev, scaler, alpha=0.3, temperature=3.0):
    """Train one epoch with both real data (hard labels) and pseudo-labeled data (soft labels)."""
    model.train(); tl,co,n=0.,0.,0
    pseudo_iter = iter(pseudo_loader)
    for x,y in tqdm(real_loader,desc="  train+PL",leave=False):
        x,y=x.to(dev,non_blocking=True),y.to(dev,non_blocking=True)
        idx=torch.randperm(x.size(0),device=x.device)
        if np.random.random()<CFG["cutmix_prob"]:
            lam=float(np.random.beta(CFG["cutmix_alpha"],CFG["cutmix_alpha"]))
        ...
        with torch.amp.autocast('cuda'):
            logits=model(xm)
            loss_real = lam*crit(logits,ya)+(1-lam)*crit(logits,yb)
        try:
            px,py_soft = next(pseudo_iter)
        except StopIteration:
            pseudo_iter = iter(pseudo_loader)
            px,py_soft = next(pseudo_iter)
        px = px.to(dev,non_blocking=True)
        py_soft = py_soft.to(dev,non_blocking=True)
        with torch.amp.autocast('cuda'):
            p_logits = model(px)
            log_p = F.log_softmax(p_logits / temperature, dim=1)
            soft_t = F.softmax(py_soft / temperature, dim=1)
            loss_kd = F.kl_div(log_p, soft_t, reduction='batchmean') * (temperature ** 2)
        loss = (1 - alpha) * loss_real + alpha * loss_kd
        opt.zero_grad(set_to_none=True); scaler.scale(loss).backward()
        scaler.unscale_(opt); nn.utils.clip_grad_norm_(model.parameters(),CFG["grad_clip_norm"])
        scaler.step(opt); scaler.update()
        if sched is not None: sched.step()
        bs=x.size(0); tl+=loss_real.item()*bs
        co+=(lam*logits.argmax(1).eq(ya).float()+(1-lam)*logits.argmax(1).eq(yb).float()).sum().item(); n+=bs
    return tl/n,co/n
...
def predict_tta(ms,loader,dev,tta=True):
    ...
    for x,_ in tqdm(loader,desc="  predict",leave=False):
        ...
        for v in vs:
            for m in ms:
                with torch.amp.autocast('cuda'): bp+=F.softmax(m(v),1)
...
def main():
    ...
    log.info("5-fold CV | %s | Fold %d | WEAK aug + pseudo-labeling | NO early stopping | 400 ep", VERSION, TARGET_FOLD)
    ...
    probs_all = []
    for f in range(5):
        d = torch.load(PSEUDO_DIR/f"eval_probs_fold{f}.pt", map_location='cpu', weights_only=True)
        probs_all.append(d['probs_tta'])
        if f == 0: pseudo_keys = d['keys']
    avg_probs = torch.stack(probs_all).mean(0)
    fold_preds = torch.stack([p.argmax(1) for p in probs_all])
    agreement = (fold_preds == fold_preds[0]).all(0)
    max_conf = avg_probs.max(1)[0]
    mask = agreement & (max_conf >= CFG["pseudo_conf_thresh"])
    pl_keys = [pseudo_keys[i] for i in range(len(pseudo_keys)) if mask[i]]
    pl_logits = avg_probs[mask]
    log.info("Pseudo-labels: %d / %d test samples (%.1f%%) with 5-fold agreement & conf>=%.0f%%",
             len(pl_keys), len(pseudo_keys), 100*len(pl_keys)/len(pseudo_keys), 100*CFG["pseudo_conf_thresh"])

    skf=StratifiedKFold(n_splits=CFG["n_folds"],shuffle=True,random_state=CFG["seed"])
    all_splits = list(skf.split(keys,labels))
    tri, vai = all_splits[TARGET_FOLD]

    log.info("="*64); log.info("FOLD %d — WEAK aug, NO early stop",TARGET_FOLD); log.info("="*64)
    ...
    tds=RadarDataset(trk,trl,train_cache,augment=True,img_size=CFG["img_size"],max_frames=CFG["max_frames"])
    vds=RadarDataset(vak,val_,train_cache,augment=False,img_size=CFG["img_size"],max_frames=CFG["max_frames"])
    ...
    pds=PseudoRadarDataset(pl_keys,pl_logits,eval_cache,augment=True,
                           img_size=CFG["img_size"],max_frames=CFG["max_frames"])
    pl=DataLoader(pds,batch_size=CFG["batch_size"],shuffle=True,num_workers=CFG["num_workers"],
                  pin_memory=True,drop_last=True,persistent_workers=False)
    log.info("Pseudo-label loader: %d samples, %d batches",len(pds),len(pl))
    ...
    ba=PARENT_DIR/f"best_fold{TARGET_FOLD}.pt"
    if ba.exists():
        ck=torch.load(ba,map_location=dev,weights_only=True)
        model.load_state_dict(ck["model_state_dict"])
        log.info("  Born-again <- %s (val %.2f%%)",ba.name,100*ck.get("val_acc",0))
    else:
        log.warning("  No born-again checkpoint at %s!", ba)

    scaler = torch.amp.GradScaler('cuda')
    crit=nn.CrossEntropyLoss(label_smoothing=CFG["label_smoothing"])
    best_acc=0.0; ckpt=out/f"best_fold{TARGET_FOLD}.pt"
    ...
    for ep in range(1,CFG["stage1_epochs"]+1):
        t0=time.time()
        trl_,tra_=train_epoch(model,tl,opt,None,crit,dev,scaler,use_mix=False)
        ...
        if va_>best_acc:
            best_acc=va_; torch.save({"fold":TARGET_FOLD,"epoch":ep,"stage":1,"model_state_dict":model.state_dict(),"val_acc":va_},ckpt)
            log.info("  ★ best %.2f%%",100*va_)
    del opt

    log.info("── Stage 2: full fine-tune + pseudo-label KD (%d ep, alpha=%.2f, T=%.1f) ──",
             CFG["stage2_epochs"], CFG["kd_alpha"], CFG["kd_temperature"])
    ...
    for ep in range(1,CFG["stage2_epochs"]+1):
        t0=time.time()
        trl_,tra_=train_epoch_kd(model,tl,pl,opt,sched,crit,dev,scaler,
                                 alpha=CFG["kd_alpha"],temperature=CFG["kd_temperature"])
        _,va_=eval_epoch(model,vl,crit,dev)
        ...
        if va_>best_acc:
            best_acc=va_
            torch.save({"fold":TARGET_FOLD,"epoch":ep,"stage":2,"model_state_dict":model.state_dict(),"val_acc":va_},ckpt)
            log.info("  ★ best %.2f%%",100*va_)
    del opt,sched
    ...
    for swa_ep in range(1,CFG["swa_epochs"]+1):
        t0=time.time()
        trl_,_=train_epoch(model,tl,swa_opt,None,crit,dev,scaler,use_mix=True)
    ...
    if swa_acc>best_acc:
        best_acc=swa_acc
        torch.save({"fold":TARGET_FOLD,"epoch":-1,"stage":3,"model_state_dict":swa_model.module.state_dict(),"val_acc":swa_acc},ckpt)
        log.info("  ★ SWA beats regular! %.2f%%",100*swa_acc)
    log.info("  Fold %d best: %.2f%%",TARGET_FOLD,100*best_acc)
    json.dump({"fold":TARGET_FOLD,"accuracy":best_acc},open(out/f"cv_fold{TARGET_FOLD}.json","w"),indent=2)

    all_accs = []
    for f in range(5):
        rf = out / f"cv_fold{f}.json"
        if rf.exists():
            all_accs.append(json.load(open(rf))["accuracy"])
    if len(all_accs) == 5:
        avg = np.mean(all_accs)
        log.info("="*64)
        log.info("ALL 5 FOLDS COMPLETE — Average: %.2f%%", 100*avg)
        for i, a in enumerate(all_accs):
            log.info("  Fold %d: %.2f%%", i, 100*a)
        log.info("="*64)
        json.dump({"version": VERSION, "fold_accs": all_accs, "average": avg},
                  open(out/"cv_5fold_results.json","w"), indent=2)
    else:
        log.info("  %d/5 folds complete so far", len(all_accs))

    del model,swa_model,swa_opt; torch.cuda.empty_cache()

    eds=RadarDataset(eval_keys,[0]*len(eval_keys),eval_cache,augment=False,
                     img_size=CFG["img_size"],max_frames=CFG["max_frames"])
    el=DataLoader(eds,batch_size=CFG["batch_size"],shuffle=False,num_workers=CFG["num_workers"],pin_memory=True,persistent_workers=False)
    ...
    pn=predict_tta([m],el,dev,tta=False); _write_sub(eval_keys,pn.argmax(1).numpy(),out/f"submission_fold{TARGET_FOLD}_no_tta.csv")
    pt=predict_tta([m],el,dev,tta=True); _write_sub(eval_keys,pt.argmax(1).numpy(),out/f"submission_fold{TARGET_FOLD}_tta.csv")
    ch=(pt.argmax(1).numpy()!=pn.argmax(1).numpy()).sum()
    log.info("TTA changed %d / %d (%.1f%%)",ch,len(eval_keys),100*ch/len(eval_keys))
    torch.save({"keys":eval_keys,"probs_tta":pt,"probs_noTTA":pn},out/f"eval_probs_fold{TARGET_FOLD}.pt")
    log.info("Fold %d done.", TARGET_FOLD)
