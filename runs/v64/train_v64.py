#!/usr/bin/env python3
"""
v64: 3ch MTI-only @ 224 + spectral threshold -90dB + born-again from v11 + 8-teacher KD + SWA.
Tests whether MTI alone (without raw RTM) is sufficient — the model focuses purely on motion
signatures. Uses same 3ch architecture as v11, so direct born-again transfer works.
...
OUT_DIR = Path("/data/slr/checkpoints_v64")
        ...
        for fold in range(CFG["n_folds"]):
            ...
            with torch.no_grad():
                for x, _, _ in loader:
                    ...
                    teacher_probs[idx:idx+p.size(0)] += p; idx += p.size(0)
            n_folds += 1; del m; torch.cuda.empty_cache()
        if n_folds > 0:
            teacher_probs /= n_folds; ensemble_probs += teacher_probs; n_teachers += 1
            log.info("    → %d folds loaded", n_folds)
    if n_teachers == 0: ensemble_probs.fill_(1.0 / CFG["n_classes"])
    else: ensemble_probs /= n_teachers
...
def main():
    ...
    log.info("KD α=%.2f τ=%.1f | SWA %d ep | 8-TEACHER KD | 3ch MTI-only",alpha,tau,CFG["swa_epochs"])
