#!/usr/bin/env python3
"""
v47: ConvNeXtV2-Tiny @ 320×320 + SWA. Fill resolution gap between 224 and 384.
Born-again from v11 (224×224) + KD from vit_v9 (CAFormer).
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v47")
...
def main():
    ...
    for fold,(tri,vai) in enumerate(skf.split(keys,labels)):
        if fold < RESUME_FROM_FOLD:
            ckpt=out/f"best_fold{fold}.pt"
            if ckpt.exists():
                prev_acc=torch.load(ckpt,map_location="cpu",weights_only=True).get("val_acc",0)
                fold_accs.append(prev_acc); log.info("FOLD %d: skip (saved %.2f%%)",fold+1,100*prev_acc)
            continue
