#!/usr/bin/env python3
"""
v37: CAFormer-S18 @ 224 with MEGA-TEACHER ENSEMBLE DISTILLATION + SWA.
Same 5-teacher mega-KD as v36 but student is CAFormer-S18.
Born-again from vit_v9.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v37")
...
CFG = {
    ...
    "drop_path_rate": 0.15, "proj_dim": 384, "drop_rate": 0.3,
...
MEGA_TEACHER_CONFIGS = [
    {"dir": V11_DIR, "cls": MultiScaleConvNeXtV2, "model_name": "convnextv2_tiny.fcmae_ft_in22k_in1k",
     "img_size": 224, "kwargs": {"drop_path_rate":0.0, "proj_dim":512, "drop_rate":0.3, "ms_samples":5}},
    {"dir": VIT_V9_DIR, "cls": MultiScaleCAFormer, "model_name": "caformer_s18.sail_in22k_ft_in1k",
     "img_size": 224, "kwargs": {"drop_path_rate":0.0, "proj_dim":384, "drop_rate":0.3, "ms_samples":5}},
    {"dir": V18_DIR, "cls": MultiScaleConvNeXtV2, "model_name": "convnextv2_tiny.fcmae_ft_in22k_in1k",
     "img_size": 384, "kwargs": {"drop_path_rate":0.0, "proj_dim":512, "drop_rate":0.3, "ms_samples":5}},
    {"dir": V25_DIR, "cls": MultiScaleConvNeXtV2, "model_name": "convnextv2_tiny.fcmae_ft_in22k_in1k",
     "img_size": 448, "kwargs": {"drop_path_rate":0.0, "proj_dim":512, "drop_rate":0.3, "ms_samples":5}},
    {"dir": V24_DIR, "cls": MultiScaleEfficientNetV2, "model_name": "tf_efficientnetv2_s.in21k_ft_in1k",
     "img_size": 384, "kwargs": {"drop_path_rate":0.0, "proj_dim":384, "drop_rate":0.3, "ms_samples":5}},
]

def precompute_mega_teacher_labels(keys, cache, configs, dev):
    log.info("Pre-computing MEGA-TEACHER soft labels (5 model groups) …")
    n = len(keys)
    nc = CFG["n_classes"]
    accumulated = torch.zeros(n, nc)
    total_models = 0

    for cfg in configs:
        td = cfg["dir"]
        if not td.exists():
            log.info("  SKIP %s (not found)", td)
            continue
        native_sz = cfg["img_size"]
        kw = {"n_classes": nc, "pretrained": False, **cfg["kwargs"]}
        models = []
        ...
        log.info("  %s @ %d: %d checkpoints", td.name, native_sz, len(models))

        ds = EvalDataset(keys, cache, img_size=native_sz, max_frames=48)
        loader = DataLoader(ds, batch_size=64 if native_sz <= 224 else 32 if native_sz <= 384 else 16,
                            shuffle=False, num_workers=CFG["num_workers"], pin_memory=True)
        all_probs = []
        with torch.no_grad():
            for x in tqdm(loader, desc=f"  {td.name}@{native_sz}", leave=False):
                x = x.to(dev, non_blocking=True)
                avg = sum(F.softmax(m(x), 1) for m in models) / len(models)
                all_probs.append(avg.cpu())
        group_probs = torch.cat(all_probs)
        accumulated += group_probs
        total_models += 1
        del models; torch.cuda.empty_cache()

    accumulated /= max(total_models, 1)
    log.info("  Mega-teacher: %d groups, avg entropy = %.3f",
             total_models, -(accumulated * (accumulated + 1e-8).log()).sum(1).mean().item())
    return {k: accumulated[i] for i, k in enumerate(keys)}

def get_llrd_params(model, base_lr, decay=0.75, wd=0.05):
    groups={}
    for name,p in model.named_parameters():
        if not p.requires_grad: continue
        if "backbone.stages.0" in name or "backbone.stem" in name: lr=base_lr*decay**4
        elif "backbone.stages.1" in name: lr=base_lr*decay**3
        elif "backbone.stages.2" in name: lr=base_lr*decay**2
        elif "backbone.stages.3" in name: lr=base_lr*decay**1
        else: lr=base_lr
...
def main():
    ...
    log.info("★★★ MEGA-TEACHER ENSEMBLE DISTILLATION — CAFormer student ★★★")
    ...
    soft_labels=precompute_mega_teacher_labels(keys,train_cache,MEGA_TEACHER_CONFIGS,dev)
    ...
    fms=[]
    for f in range(CFG["n_folds"]):
        m=MultiScaleCAFormer(CFG["model_name"],CFG["n_classes"],pretrained=False,drop_path_rate=0.0,
