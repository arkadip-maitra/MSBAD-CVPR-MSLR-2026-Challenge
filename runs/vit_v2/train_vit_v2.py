...
ViT v2: DeiT3-Small (ImageNet-22k pretrained) with 224×224 resize, EMA, LLRD.

Key fixes over ViT v1 (30% → should reach ~75-82%):
  - Resize to 224×224 so pretrained position embeddings work as designed
    (v1's 14×14→4×16 pos-embed interpolation destroyed pretrained structure)
  - DeiT3-Small (21.7M) — stronger backbone + ImageNet-22k pretraining
  - EMA, LLRD, 10-way TTA, all v3 augmentations

Usage:
    conda run -n slr python /data/slr/train_vit_v2.py
"""
...
# ---------------------------------------------------------------------------
OUT_DIR = Path("/data/slr/checkpoints_vit_v2")
...
class RadarDataset(Dataset):
    ...
    def _apply_aug(self, x):
        ...
        if np.random.random() < 0.3:
            shift = np.random.randint(
                -CFG["time_shift_max"], CFG["time_shift_max"] + 1
            )
    ...
    def __getitem__(self, idx):
        ...
        # Resize (3, 48, 256) → (3, 224, 224) via bilinear interpolation
        t = F.interpolate(
...
def create_model(n_cls=126, pretrained=True):
...
def get_llrd_param_groups(model, base_lr, decay, wd):
    """LLRD for DeiT3/ViT: lower LR for earlier transformer blocks."""
...
def _tta_transforms(x: torch.Tensor) -> list[torch.Tensor]:
    """10 augmented views for TTA on (B, 3, 224, 224) input."""
    views = [x]
...
def main():
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(keys, labels)):
        ...
        model = create_model(CFG["n_classes"], CFG["pretrained"]).to(dev)
    ...
    for fold in range(CFG["n_folds"]):
        m = create_model(n_cls=CFG["n_classes"], pretrained=False).to(dev)
        ckpt = torch.load(
