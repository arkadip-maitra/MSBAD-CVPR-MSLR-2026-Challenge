#!/usr/bin/env python3
"""v34: SwinV2-Tiny @ 224×224 + aggressive KD (α=0.1) + longer training — rescue attempt."""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v34")
...
class MultiScaleModel(nn.Module):
    ...
    def forward(self, x):
        ...
        fixed = []
        for f, gem, proj, dim in zip(feats, self.stage_gems, self.stage_projs,
                                      self.backbone.feature_info.channels()):
            if f.dim() == 4 and f.shape[1] != dim:
                f = f.permute(0, 3, 1, 2)
            fixed.append(proj(gem(f).flatten(1)))
        stacked = torch.stack(fixed, dim=1)
...
def get_llrd_params(model, base_lr, decay=0.75, wd=0.05):
    ...
    for name,p in model.named_parameters():
        ...
        depth = None
        for i in range(8):
            if f"layers.{i}." in name or f"stages.{i}." in name or f"blocks.{i}." in name or f"stages_{i}" in name:
                depth = i; break
        if depth is not None:
            max_d = max(1, max(i for i in range(8) if any(f"{k}.{i}." in n for n,_ in model.named_parameters() for k in ["layers","stages","blocks"])))
            lr = base_lr * decay ** (max_d - depth)
        elif any(k in name for k in ["patch_embed","stem","conv_stem","bn1"]):
            lr = base_lr * decay ** 5
        else:
            lr = base_lr
...
def main():
    ...
    log.info("★★★ %s @ %d ★★★",MODEL_NAME,IMG_SIZE)

    train_samples,_=collect_train_samples(CFG["data_dir"])
    ...
    fms=[]
    for f in range(CFG["n_folds"]):
        m=MultiScaleModel(MODEL_NAME,CFG["n_classes"],pretrained=False,drop_path_rate=0.0,
