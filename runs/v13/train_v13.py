...
CNN v13: ConvNeXtV2-Tiny born-again from v11 + triple cross-arch KD (CAFormer + EfficientNetV2 teachers)
"""
...
OUT_DIR = Path("/data/slr/checkpoints_v13")
...
def main():
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(keys, labels)):
        ...
        born_again_path = V11_DIR / f"best_fold{fold}.pt"
        if born_again_path.exists():
            load_born_again(model, born_again_path, dev)
        else:
            log.info("  No v11 checkpoint for fold %d, using ImageNet init", fold)
    ...
    log.info("Loading v13 checkpoints for ensemble …")
    models_v13 = []
    for fold in range(CFG["n_folds"]):
    ...
    probs_no = predict_ensemble_tta(models_v13, eval_loader, dev, use_tta=False)
    ...
    probs_v13 = predict_ensemble_tta(models_v13, eval_loader, dev, use_tta=True)
    preds_v13 = probs_v13.argmax(dim=1).numpy()
    _write_submission(eval_keys, preds_v13, out / "submission_tta.csv")
    log.info("Submission (TTA) → %s", out / "submission_tta.csv")
    changed = (preds_v13 != preds_no).sum()
    log.info("TTA changed %d / %d (%.1f%%)", changed, len(preds_v13),
             100 * changed / len(preds_v13))

    torch.save({"keys": eval_keys, "probs": probs_v13},
               out / "eval_probs_tta.pt")
    log.info("Saved v13 TTA probs → %s", out / "eval_probs_tta.pt")
