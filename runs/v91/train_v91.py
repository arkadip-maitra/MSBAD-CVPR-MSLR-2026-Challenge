#!/usr/bin/env python3
"""
v91: CosineAnnealingWarmRestarts — periodic LR bumps to escape local minima.
Born-again from v67 (best=90.98%). 5 cycles of 40 epochs = 200 total.
Saves best checkpoint from any cycle. Slightly stronger augmentation.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v91")
...
CFG = {
    ...
    "stage2_T_0": 40, "stage2_T_mult": 1,
    "swa_epochs": 30, "swa_lr": 3e-6,
    "weight_decay": 0.05, "warmup_pct": 0.05,
    "label_smoothing": 0.1, "early_stopping_patience": 50,
...
def main():
    ...
    log.info("CosineWarmRestarts | Born-again v67 | Fold %d", TARGET_FOLD)
    ...
    log.info("="*64); log.info("FOLD %d — CosineWarmRestarts",TARGET_FOLD); log.info("="*64)
    ...
    log.info("── Stage 2: CosineAnnealingWarmRestarts T_0=%d T_mult=%d (%d ep) ──",
             CFG["stage2_T_0"],CFG["stage2_T_mult"],CFG["stage2_epochs"])
    ...
    sched=torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(opt, T_0=T_0_steps, T_mult=CFG["stage2_T_mult"], eta_min=1e-7)
    pat=0
    for ep in range(1,CFG["stage2_epochs"]+1):
        t0=time.time()
        trl_,tra_=train_epoch(model,tl,opt,sched,crit,dev,scaler,use_mix=True)
        _,va_=eval_epoch(model,vl,crit,dev)
        cycle = (ep - 1) // CFG["stage2_T_0"] + 1
        log.info("  S2 Ep %3d/%d (C%d)  loss=%.4f acc=%.1f%%  val=%.1f%%  lr=%.2e  %4.0fs",
                 ep,CFG["stage2_epochs"],cycle,trl_,100*tra_,100*va_,opt.param_groups[-1]["lr"],time.time()-t0)
        if va_>best_acc:
            best_acc=va_; pat=0
