#!/usr/bin/env python3
"""
v61: SwinV2-CR-Small @ 224 (49M params) — single fold, 8-teacher KD.
Same channel dims [96,192,384,768] as ConvNeXtV2-Tiny → can load head weights from v11.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v61")
...
CFG = {
    ...
    "stage1_epochs": 5, "stage1_lr": 1.5e-4,
    "stage2_epochs": 200, "stage2_lr": 4e-5,
    "swa_epochs": 25, "swa_lr": 4e-6,
    "weight_decay": 0.06, "warmup_pct": 0.06,
    "label_smoothing": 0.12, "early_stopping_patience": 50,
    "grad_clip_norm": 1.0, "llrd_decay": 0.85,
    "kd_alpha": 0.8, "kd_tau": 5.0,
    ...
    "random_erase_prob": 0.22, "circ_shift_prob": 0.3, "circ_shift_max": 8,
    "amp_scale_prob": 0.5, "amp_scale_lo": 0.8, "amp_scale_hi": 1.2,
    "model_name": "swinv2_cr_small_224.sw_in1k",
    "drop_path_rate": 0.25, "proj_dim": 512, "drop_rate": 0.35,
...
TEACHER_CONFIGS = [
    {"dir": V11_DIR, "cls": MultiScaleModel, "model_name": "convnextv2_tiny.fcmae_ft_in22k_in1k",
    ...
    {"dir": V18_DIR, "cls": MultiScaleModel, "model_name": "convnextv2_tiny.fcmae_ft_in22k_in1k",
     "kwargs": {"drop_path_rate":0.0,"proj_dim":512,"drop_rate":0.3,"ms_samples":5}, "img_size": 384},
    {"dir": V25_DIR, "cls": MultiScaleModel, "model_name": "convnextv2_tiny.fcmae_ft_in22k_in1k",
    ...
    {"dir": V38_DIR, "cls": MultiScaleModel, "model_name": "convnextv2_tiny.fcmae_ft_in22k_in1k",
     "kwargs": {"drop_path_rate":0.0,"proj_dim":512,"drop_rate":0.3,"ms_samples":5}, "img_size": 512},
    {"dir": V47_DIR, "cls": MultiScaleModel, "model_name": "convnextv2_tiny.fcmae_ft_in22k_in1k",
     "kwargs": {"drop_path_rate":0.0,"proj_dim":512,"drop_rate":0.3,"ms_samples":5}, "img_size": 320},
    {"dir": V52_DIR, "cls": MultiScaleModel, "model_name": "convnextv2_tiny.fcmae_ft_in22k_in1k",
     "kwargs": {"drop_path_rate":0.0,"proj_dim":512,"drop_rate":0.3,"ms_samples":5}, "img_size": 384},
]

def load_head_from_v11(model, fold, dev):
    """Load head weights (gems, projs, attn, classifier) from v11 — same channel dims."""
    cp = V11_DIR / f"best_fold{fold}.pt"
    if not cp.exists():
        log.info("  v11 fold %d checkpoint not found, skipping head transfer", fold)
        return
    sd_src = torch.load(cp, map_location=dev, weights_only=True)["model_state_dict"]
    sd_tgt = model.state_dict()
    loaded = 0
    for k in sd_tgt:
        if "backbone" in k:
            continue
        if k in sd_src and sd_src[k].shape == sd_tgt[k].shape:
            sd_tgt[k] = sd_src[k]
            loaded += 1
    model.load_state_dict(sd_tgt)
    log.info("  Head transfer from v11 fold %d: %d/%d weights loaded", fold, loaded,
             sum(1 for k in sd_tgt if "backbone" not in k))
...
def precompute_teacher_labels(keys, cache, configs, dev):
    ...
    for ti, cfg in enumerate(configs):
        ...
        ds = RadarDataset(keys, [0]*n_samples, cache, augment=False,
                         img_size=t_img_size, max_frames=CFG["max_frames"])
...
def get_llrd_params(model, base_lr, decay=0.85, wd=0.06):
    groups={}
    for name,p in model.named_parameters():
        if not p.requires_grad: continue
        depth = None
        if "backbone" in name:
            for tag in ["patch_embed", "stem", "stages_0", "stages.0", "layers.0"]:
                if tag in name: depth = 5; break
            if depth is None:
                for tag in ["stages_1", "stages.1", "layers.1"]:
                    if tag in name: depth = 4; break
            if depth is None:
                for tag in ["stages_2", "stages.2", "layers.2"]:
                    if tag in name: depth = 3; break
            if depth is None:
                for tag in ["stages_3", "stages.3", "layers.3"]:
                    if tag in name: depth = 2; break
            if depth is None:
                depth = 3
        lr = base_lr * (decay ** depth) if depth is not None else base_lr
        w = 0.0 if p.ndim < 2 else wd
        key = (lr, w)
        if key not in groups: groups[key] = {"params": [], "lr": lr, "weight_decay": w}
        groups[key]["params"].append(p)
    r = list(groups.values())
    lrs = sorted(set(g["lr"] for g in r))
    log.info("LLRD: %d groups, LR [%.2e … %.2e]", len(r), min(lrs), max(lrs))
    return r
...
def train_epoch(model, loader, opt, sched, crit, dev, alpha, tau, use_mix=True, scaler=None):
...
def eval_epoch(model, loader, crit, dev):
    ...
    for x,y,_ in tqdm(loader,desc="  val  ",leave=False):
        ...
        with torch.amp.autocast("cuda"):
            lo=model(x)
        tl+=crit(lo,y).item()*x.size(0); co+=lo.argmax(1).eq(y).sum().item(); n+=x.size(0)
    return tl/n,co/n
...
def main():
    ...
    log.info("Student: SwinV2-CR-Small@224 (49M) — SINGLE FOLD %d", CFG["run_fold"])
    ...
    log.info("KD α=%.2f τ=%.1f | SWA %d ep | SINGLE FOLD | Head transfer from v11",alpha,tau,CFG["swa_epochs"])

    scaler = torch.amp.GradScaler("cuda")

    skf=StratifiedKFold(n_splits=CFG["n_folds"],shuffle=True,random_state=CFG["seed"])
    fold = CFG["run_fold"]
    for fi,(tri,vai) in enumerate(skf.split(keys,labels)):
        if fi != fold: continue
        log.info("="*64); log.info("FOLD %d (single fold mode)",fold); log.info("="*64)
        ...
        for ep in range(1,CFG["stage1_epochs"]+1):
            t0=time.time()
            trl_,tra_=train_epoch(model,tl,opt,sched,crit,dev,alpha,tau,use_mix=False,scaler=scaler)
        ...
        for ep in range(1,CFG["stage2_epochs"]+1):
            t0=time.time()
            trl_,tra_=train_epoch(model,tl,opt,sched,crit,dev,alpha,tau,use_mix=True,scaler=scaler)
            _,va_=eval_epoch(model,vl,crit,dev)
        ...
        for swa_ep in range(1,CFG["swa_epochs"]+1):
            t0=time.time()
            trl_,_=train_epoch(model,tl,swa_opt,None,crit,dev,alpha,tau,use_mix=True,scaler=scaler)
        ...
        log.info("  Fold %d best: %.2f%%",fold,100*best_acc)
        del model,swa_model,swa_opt; torch.cuda.empty_cache()

    log.info("="*64); log.info("RESULT: Fold %d accuracy = %.2f%%", fold, 100*best_acc); log.info("="*64)
    json.dump({"fold":fold,"accuracy":float(best_acc)},open(out/"cv_results.json","w"),indent=2)
    ...
    m=MultiScaleModel(CFG["model_name"],CFG["n_classes"],pretrained=False,drop_path_rate=0.0,
                      proj_dim=CFG["proj_dim"],drop_rate=CFG["drop_rate"],ms_samples=CFG["ms_dropout_samples"]).to(dev)
    m.load_state_dict(torch.load(out/f"best_fold{fold}.pt",map_location=dev,weights_only=True)["model_state_dict"])
