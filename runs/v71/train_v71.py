#!/usr/bin/env python3
"""
v71: CLIP contrastive learning (InfoNCE) — what TRACE used for 99.3%.
Project radar features into CLIP embedding space, pull toward correct
text embedding, push away from others. Born-again from v66.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v71")
V66_DIR = Path("/data/slr/checkpoints_v66")
...
class CLIPContrastiveConvNeXtV2(nn.Module):
    """ConvNeXtV2 with CLIP contrastive projection head."""
    ...
    def forward(self, x, return_clip_feat=False):
        ...
        if return_clip_feat:
            clip_feat = F.normalize(self.clip_proj(pooled), dim=-1)
...
def compute_text_embeddings(idx_to_cls, dev):
    ...
    log.info("CLIP text embeddings: mean_sim=%.3f, min_sim=%.3f",
             sim[mask].mean().item(), sim[mask].min().item())
...
def get_llrd_params(model, base_lr, decay=0.75, wd=0.05):
    ...
    for name,p in model.named_parameters():
        ...
        elif "clip_proj" in name:           lr=base_lr*2.0
        else: lr=base_lr
...
def infonce_loss(radar_feats, labels, text_embs, tau=0.07):
    """InfoNCE: pull radar embedding toward its class text embedding, push away from others."""
    text_for_batch = text_embs[labels]
    logits = radar_feats @ text_embs.T / tau
    return F.cross_entropy(logits, labels)

def train_epoch(model, loader, opt, sched, crit, dev, scaler, text_embs, use_mix=True):
    ...
    for x,y in tqdm(loader,desc="  train",leave=False):
        ...
        with torch.amp.autocast('cuda'):
            logits, clip_feat = model(xm, return_clip_feat=True)
            ce_loss = lam*crit(logits,ya)+(1-lam)*crit(logits,yb)
            nce_loss = lam*infonce_loss(clip_feat, ya, text_embs, CLIP_TAU) + \
                       (1-lam)*infonce_loss(clip_feat, yb, text_embs, CLIP_TAU)
            loss = (1 - CLIP_ALPHA) * ce_loss + CLIP_ALPHA * nce_loss
...
def main():
    ...
    log.info("CLIP CONTRASTIVE (InfoNCE) | alpha=%.2f tau=%.2f | Born-again v66 | Fold %d",
             CLIP_ALPHA, CLIP_TAU, TARGET_FOLD)
    ...
    text_embs = compute_text_embeddings(idx_to_cls, dev)

    skf=StratifiedKFold(n_splits=CFG["n_folds"],shuffle=True,random_state=CFG["seed"])
    for fold,(tri,vai) in enumerate(skf.split(keys,labels)):
        if fold == TARGET_FOLD: break
    log.info("="*64); log.info("FOLD %d — CLIP CONTRASTIVE, born-again v66",TARGET_FOLD); log.info("="*64)
    ...
    for ep in range(1,CFG["stage1_epochs"]+1):
        t0=time.time()
        trl_,tra_=train_epoch(model,tl,opt,sched,crit,dev,scaler,text_embs,use_mix=False)
    ...
    for ep in range(1,CFG["stage2_epochs"]+1):
        t0=time.time()
        trl_,tra_=train_epoch(model,tl,opt,sched,crit,dev,scaler,text_embs,use_mix=True)
        _,va_=eval_epoch(model,vl,crit,dev)
    ...
    for swa_ep in range(1,CFG["swa_epochs"]+1):
        t0=time.time()
        trl_,_=train_epoch(model,tl,swa_opt,None,crit,dev,scaler,text_embs,use_mix=True)
    ...
    m=CLIPContrastiveConvNeXtV2(CFG["model_name"],CFG["n_classes"],pretrained=False,drop_path_rate=0.0,
