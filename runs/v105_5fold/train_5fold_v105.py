#!/usr/bin/env python3
"""
v105: 5-fold CV — ConvNeXtV2-BASE born-again from v104. Round 2.
OneCycleLR with lower peak LR for refinement.
Usage: python train_5fold_v105.py --fold 0 --gpu 0
"""
...
VERSION = "v105"
PARENT_DIR = Path("/data/slr/checkpoints_v104")
...
class MultiSampleDropout(nn.Module):
    def __init__(self,inf,outf,n_samples=5,drop_rate=0.4):
...
class MultiScaleConvNeXtV2(nn.Module):
    def __init__(self, model_name, n_classes=126, pretrained=True,
                 drop_path_rate=0.3, proj_dim=512, drop_rate=0.4, ms_samples=5):
        super().__init__()
...
def get_llrd_params(model, base_lr, decay=0.8, wd=0.05):
    groups={}
...
def main():
    ...
    log.info("5-fold CV | %s | Fold %d | ConvNeXtV2-BASE | Born-again from v104 | OneCycleLR", VERSION, TARGET_FOLD)
    ...
    log.info("="*64); log.info("FOLD %d — ConvNeXtV2-BASE born-again from v104",TARGET_FOLD); log.info("="*64)
    ...
    log.info("  Model params: %.1fM", nparams)
    ...
    if ba.exists():
        ...
        log.warning("  No born-again checkpoint at %s, training from pretrained!", ba)
    ...
    log.info("── Stage 2: OneCycleLR (%d ep, lr=%.1e, patience %d) ──",
             CFG["stage2_epochs"], CFG["stage2_lr"], CFG["early_stopping_patience"])
    ...
    sched=OneCycleLR(opt,max_lr=[g["lr"] for g in pg],
    ...
    for ep in range(1,CFG["stage2_epochs"]+1):
        ...
        log.info("  S2 Ep %3d/%d  loss=%.4f acc=%.1f%%  val=%.1f%%  lr=%.2e  %4.0fs",
                 ep,CFG["stage2_epochs"],trl_,100*tra_,100*va_,cur_lr,time.time()-t0)
        if va_>best_acc:
            best_acc=va_; pat=0
    ...
    el=DataLoader(eds,batch_size=CFG["batch_size"],shuffle=False,num_workers=CFG["num_workers"],pin_memory=True,persistent_workers=True)
