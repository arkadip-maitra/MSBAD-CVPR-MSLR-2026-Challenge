...
ViT v5: Swin-Small + Two-Stage + Knowledge Distillation from CNN v6 ensemble.

Improvements over ViT v4 (83.14%):
  - KNOWLEDGE DISTILLATION: 5-model CNN v6 teacher ensemble produces
    soft labels. Loss = α·CE + (1-α)·τ²·KL — distills CNN's 86.20% knowledge
  - REMOVED SWA: hurt every fold in v4 (SWA < best individual)
  - ADDED EARLY STOPPING: patience=30 in Stage 2 (v4 ran full 150 epochs)
  - max_frames=48 (matches CNN teacher for consistent KD)
  - All v4 keeps: Swin-Small, two-stage, GeM, MultiSampleDropout, LLRD

Usage:
    conda run -n slr python /data/slr/train_vit_v5.py
"""
...
OUT_DIR = Path("/data/slr/checkpoints_vit_v5")
V6_CNN_DIR = Path("/data/slr/checkpoints_v6")
...
CFG = {
    ...
    "pretrained": True,
    "seed": 42,
    # CNN teacher config (for loading v6 checkpoints)
...
class ConvNeXtGeM(nn.Module):
    """For loading CNN v6 teacher checkpoints."""
...
def precompute_teacher_labels(keys, cache, dev):
    """Pre-compute soft labels from CNN v6 (+ v4) ensemble."""
    log.info("Pre-computing teacher soft labels from CNN ensemble …")
    ...
    teacher_models = []
    for tdir in [V6_CNN_DIR, V4_CNN_DIR]:
        if not tdir.exists():
            continue
    ...
    log.info("  Loaded %d CNN teacher models", n_teachers)
...
# ===========================================================================
#  MIXUP + CUTMIX (with soft label support)
# ===========================================================================
...
def main():
    ...
    # ---- Pre-compute teacher soft labels from CNN ensemble ----------------
    ...
    log.info("Loading ViT v5 checkpoints …")
    vit_models = []
    for fold in range(CFG["n_folds"]):
    ...
    # ---- Cross-architecture ensemble with CNN v6 --------------------------
    if V6_CNN_DIR.exists() and (V6_CNN_DIR / "best_fold0.pt").exists():
        log.info("Loading CNN v6 checkpoints for ViT+CNN ensemble …")
        cnn_models = []
        for fold in range(CFG["n_folds"]):
            m = ConvNeXtGeM(
                CFG["cnn_model_name"], CFG["n_classes"], pretrained=False
            ...
            ckpt = torch.load(
                V6_CNN_DIR / f"best_fold{fold}.pt", map_location=dev,
                weights_only=True,
            )
            m.load_state_dict(ckpt["model_state_dict"])
            cnn_models.append(m)
        ...
        _write_submission(test_keys, preds_ens, out / "submission_vit_cnn_ensemble.csv")
        log.info("Submission (ViT+CNN ensemble) → %s",
                 out / "submission_vit_cnn_ensemble.csv")
        diff = (preds_ens != preds_vit).sum()
        log.info("ViT+CNN ensemble differs from ViT-only in %d predictions", diff)
