#!/usr/bin/env python3
"""
v88: Different training seed (137 vs 42) — different optimization trajectory
and augmentation randomness while keeping the same data split (StratifiedKFold seed=42).
Born-again from v66. Tests if model can find a different local minimum.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v88")
...
CFG = {
    ...
    "ms_dropout_samples": 5, "seed": 42, "train_seed": 137,
}
...
def main():
    ...
    torch.manual_seed(CFG["train_seed"]); np.random.seed(CFG["train_seed"]); torch.backends.cudnn.benchmark=True
    dev=torch.device("cuda:0"); log.info("GPU: %s",torch.cuda.get_device_name(0))
    log.info("Config: %s",json.dumps(CFG,indent=2))
    log.info("Different train seed=137 | born-again v66 | Fold %d", TARGET_FOLD)
    ...
    log.info("="*64); log.info("FOLD %d — Different seed (train_seed=137, fold_seed=42)",TARGET_FOLD); log.info("="*64)
