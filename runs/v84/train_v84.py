#!/usr/bin/env python3
"""
v84: CAFormer-S36 — MetaFormer architecture with pooling in early stages and
attention in later stages. Stage dims [64,128,320,512], ImageNet-1k pretrained.
No born-again (different dims). Extended training.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v84")
...
CFG = {
    ...
    "model_name": "caformer_s36.sail_in1k",
...
def get_llrd_params(model, base_lr, decay=0.75, wd=0.05):
    ...
    for name,p in model.named_parameters():
        ...
        if "backbone.stem" in name or "backbone.patch_embed" in name or "backbone.downsample" in name:
            lr=base_lr*decay**5
...
def main():
    ...
    log.info("CAFormer-S36 | no born-again | Fold %d", TARGET_FOLD)
    ...
    log.info("="*64); log.info("FOLD %d — CAFormer-S36 backbone",TARGET_FOLD); log.info("="*64)
    ...
    # No born-again — different stage dims from v66
    log.info("  No born-again (different stage dims), ImageNet pretrained only")
