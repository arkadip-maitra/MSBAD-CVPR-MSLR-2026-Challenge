#!/usr/bin/env python3
"""
v63: 6ch RTM+MTI @ 224 + spectral threshold -90dB + born-again from v11 + 8-teacher KD + SWA.
Paper insight: MTI (Moving Target Indication) removes static clutter, keeping only motion
signatures critical for sign recognition. TRACE achieved 99.3% radar-only with RDM+MTI.
Single fold (fold 0) for fast iteration.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v63")
V11_DIR = Path("/data/slr/checkpoints_v11")
VIT_V9_DIR = Path("/data/slr/checkpoints_vit_v9")
V18_DIR = Path("/data/slr/checkpoints_v18")
V25_DIR = Path("/data/slr/checkpoints_v25")
V24_DIR = Path("/data/slr/checkpoints_v24")
V38_DIR = Path("/data/slr/checkpoints_v38")
V47_DIR = Path("/data/slr/checkpoints_v47")
V52_DIR = Path("/data/slr/checkpoints_v52")
OUT_DIR.mkdir(parents=True, exist_ok=True)
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s",
                    handlers=[logging.FileHandler(OUT_DIR/"training.log"), logging.StreamHandler()])
log = logging.getLogger(__name__)

SPECTRAL_THRESHOLD = -90.0
INPUT_MODE = "rtm_mti"  # 6 channels: 3 RTM + 3 MTI
TARGET_FOLD = 0
...
class RadarDataset(Dataset):
    def __init__(self, keys, labels, cache, augment=False, soft_labels=None,
                 img_size=224, max_frames=48, mode='rtm'):
    ...
    def _apply_aug(self, x):
        ...
        if np.random.random()<CFG["channel_shuffle_prob"]:
            ...
            if is_6ch:
                x = np.concatenate([x[:3][perm], x[3:][perm]], axis=0)
            else:
                x = x[perm]
        if np.random.random()<CFG["channel_drop_prob"]:
            di = np.random.randint(0, 3)
            x[di] = 0.0
            if is_6ch: x[di+3] = 0.0
    ...
    def __getitem__(self, idx):
        ...
        x=np.maximum(x, SPECTRAL_THRESHOLD)
        if self.mode == 'rtm':
            x = (x - _MEANS) / _STDS
        elif self.mode == 'mti':
            x = (x - x.mean(axis=1, keepdims=True)) / _STDS
        elif self.mode == 'rtm_mti':
            mti = x - x.mean(axis=1, keepdims=True)
            x = np.concatenate([(x - _MEANS) / _STDS, mti / _STDS], axis=0)
...
TEACHER_CONFIGS = [
    {"dir": V11_DIR, "cls": MultiScaleConvNeXtV2, "model_name": "convnextv2_tiny.fcmae_ft_in22k_in1k",
     "kwargs": {"drop_path_rate":0.0,"proj_dim":512,"drop_rate":0.3,"ms_samples":5}, "img_size": 224},
    {"dir": VIT_V9_DIR, "cls": MultiScaleCAFormer, "model_name": "caformer_s18.sail_in22k_ft_in1k",
     "kwargs": {"drop_path_rate":0.0,"proj_dim":384,"drop_rate":0.3,"ms_samples":5}, "img_size": 224},
    {"dir": V18_DIR, "cls": MultiScaleConvNeXtV2, "model_name": "convnextv2_tiny.fcmae_ft_in22k_in1k",
     "kwargs": {"drop_path_rate":0.0,"proj_dim":512,"drop_rate":0.3,"ms_samples":5}, "img_size": 384},
    {"dir": V25_DIR, "cls": MultiScaleConvNeXtV2, "model_name": "convnextv2_tiny.fcmae_ft_in22k_in1k",
     "kwargs": {"drop_path_rate":0.0,"proj_dim":512,"drop_rate":0.3,"ms_samples":5}, "img_size": 448},
    {"dir": V24_DIR, "cls": MultiScaleEfficientNetV2, "model_name": "tf_efficientnetv2_s.in21k_ft_in1k",
     "kwargs": {"drop_path_rate":0.0,"proj_dim":384,"drop_rate":0.3,"ms_samples":5}, "img_size": 384},
    {"dir": V38_DIR, "cls": MultiScaleConvNeXtV2, "model_name": "convnextv2_tiny.fcmae_ft_in22k_in1k",
     "kwargs": {"drop_path_rate":0.0,"proj_dim":512,"drop_rate":0.3,"ms_samples":5}, "img_size": 512},
    {"dir": V47_DIR, "cls": MultiScaleConvNeXtV2, "model_name": "convnextv2_tiny.fcmae_ft_in22k_in1k",
     "kwargs": {"drop_path_rate":0.0,"proj_dim":512,"drop_rate":0.3,"ms_samples":5}, "img_size": 320},
    {"dir": V52_DIR, "cls": MultiScaleConvNeXtV2, "model_name": "convnextv2_tiny.fcmae_ft_in22k_in1k",
     "kwargs": {"drop_path_rate":0.0,"proj_dim":512,"drop_rate":0.3,"ms_samples":5}, "img_size": 384},
]

def expand_stem_to_6ch(model, dev):
    """Expand 3ch stem to 6ch by duplicating weights for MTI channels."""
    old = model.backbone.stem_0
    new = nn.Conv2d(6, old.out_channels, old.kernel_size, old.stride, old.padding,
    ...
    model.backbone.stem_0 = new.to(dev)
    log.info("  Stem expanded 3ch → 6ch (duplicated weights for MTI)")
...
def precompute_teacher_labels(keys, cache, configs, dev):
    log.info("Pre-computing 8-teacher ensemble soft labels …")
    n_samples = len(keys)
    ensemble_probs = torch.zeros(n_samples, CFG["n_classes"])
    n_teachers = 0
    for ti, cfg in enumerate(configs):
        td = cfg["dir"]
        if not td.exists():
            log.info("  [%d/%d] %s — MISSING, skip", ti+1, len(configs), td)
            continue
        t_img_size = cfg.get("img_size", 224)
        log.info("  [%d/%d] %s @ %d×%d", ti+1, len(configs), td.name, t_img_size, t_img_size)
        ds = RadarDataset(keys, [0]*n_samples, cache, augment=False,
                         img_size=t_img_size, max_frames=CFG["max_frames"], mode='rtm')
        bs = max(8, 96 // max(1, (t_img_size // 224) ** 2))
        ...
        teacher_probs = torch.zeros(n_samples, CFG["n_classes"])
        n_folds = 0
        for fold in range(CFG["n_folds"]):
            cp = td / f"best_fold{fold}.pt"
            ...
            with torch.no_grad():
                for x, _, _ in loader:
                    ...
                    with torch.amp.autocast('cuda'):
                        p = F.softmax(m(x), 1).cpu()
                    teacher_probs[idx:idx+p.size(0)] += p
                    idx += p.size(0)
            n_folds += 1
            del m; torch.cuda.empty_cache()
        if n_folds > 0:
            teacher_probs /= n_folds
            ensemble_probs += teacher_probs
            n_teachers += 1
            log.info("    → %d folds loaded", n_folds)
    if n_teachers == 0:
        ensemble_probs.fill_(1.0 / CFG["n_classes"])
    else:
        ensemble_probs /= n_teachers
    log.info("  Ensemble from %d teachers → soft labels for %d samples", n_teachers, n_samples)
    return {k: ensemble_probs[i] for i, k in enumerate(keys)}
...
def train_epoch(model, loader, opt, sched, crit, dev, alpha, tau, scaler, use_mix=True):
...
def _tta(x):
    is_6ch = x.shape[1] == 6
    augs = [x, x.flip(2), x.roll(-4,2), x.roll(4,2),
            x+.05*torch.randn_like(x), x+.08*torch.randn_like(x), x*.9, x*1.1]
    if is_6ch:
        augs += [x[:,[1,2,0,4,5,3],:,:], x[:,[2,0,1,5,3,4],:,:]]
    else:
        augs += [x[:,[1,2,0],:,:], x[:,[2,0,1],:,:]]
    return augs
...
def main():
    ...
    log.info("Input mode: %s | Spectral threshold: %.0f dB | Fold: %d", INPUT_MODE, SPECTRAL_THRESHOLD, TARGET_FOLD)
    ...
    # Log MTI statistics
    sample_key = keys[0]
    sample_x = train_cache[sample_key]
    sample_x_thr = np.maximum(sample_x, SPECTRAL_THRESHOLD)
    sample_mti = sample_x_thr - sample_x_thr.mean(axis=1, keepdims=True)
    log.info("Sample RTM range: [%.1f, %.1f], MTI range: [%.1f, %.1f]",
             sample_x.min(), sample_x.max(), sample_mti.min(), sample_mti.max())
    n_thr = (sample_x < SPECTRAL_THRESHOLD).sum()
    log.info("Thresholded %.1f%% of values in sample (below %.0f dB)",
             100*n_thr/sample_x.size, SPECTRAL_THRESHOLD)

    soft_labels=precompute_teacher_labels(keys,train_cache,TEACHER_CONFIGS,dev)
    alpha,tau=CFG["kd_alpha"],CFG["kd_tau"]
    log.info("KD α=%.2f τ=%.1f | SWA %d ep | 8-TEACHER KD | 6ch RTM+MTI",alpha,tau,CFG["swa_epochs"])

    skf=StratifiedKFold(n_splits=CFG["n_folds"],shuffle=True,random_state=CFG["seed"])
    for fold,(tri,vai) in enumerate(skf.split(keys,labels)):
        if fold == TARGET_FOLD: break
    log.info("="*64); log.info("FOLD %d (single fold mode)",TARGET_FOLD); log.info("="*64)
    ...
    tds=RadarDataset(trk,trl,train_cache,augment=True,soft_labels=soft_labels,
                     img_size=CFG["img_size"],max_frames=CFG["max_frames"],mode=INPUT_MODE)
    ...
    if ba.exists():
        load_born_again(model,ba,dev)
    if INPUT_MODE == 'rtm_mti':
        expand_stem_to_6ch(model, dev)
    ...
    for ep in range(1,CFG["stage1_epochs"]+1):
        t0=time.time()
        trl_,tra_=train_epoch(model,tl,opt,sched,crit,dev,alpha,tau,scaler,use_mix=False)
    ...
    for ep in range(1,CFG["stage2_epochs"]+1):
        t0=time.time()
        trl_,tra_=train_epoch(model,tl,opt,sched,crit,dev,alpha,tau,scaler,use_mix=True)
        _,va_=eval_epoch(model,vl,crit,dev)
    ...
    for swa_ep in range(1,CFG["swa_epochs"]+1):
        t0=time.time()
        trl_,_=train_epoch(model,tl,swa_opt,None,crit,dev,alpha,tau,scaler,use_mix=True)
