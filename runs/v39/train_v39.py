#!/usr/bin/env python3
"""
v39: EfficientNetV2-S @ 384 generation 2 + extended training.
Born-again chain: v12@224 → v24@384 → v39@384 (gen 2).
Longer training (120 ep), higher patience (40), slightly lower LR.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v39")
...
CFG = {
    ...
    "n_folds": 5, "batch_size": 24, "num_workers": 4,
    "stage1_epochs": 5, "stage1_lr": 2e-4,
    "stage2_epochs": 120, "stage2_lr": 3e-5,
    ...
    "drop_path_rate": 0.2, "proj_dim": 384, "drop_rate": 0.3,
...
def get_llrd_params_effnet(model, base_lr, decay=0.75, wd=0.05):
    groups={}
    for name,p in model.named_parameters():
        if not p.requires_grad: continue
        if any(s in name for s in ["backbone.conv_stem","backbone.bn1","backbone.blocks.0"]): lr=base_lr*decay**6
        elif "backbone.blocks.1" in name: lr=base_lr*decay**5
        elif "backbone.blocks.2" in name: lr=base_lr*decay**4
        elif "backbone.blocks.3" in name: lr=base_lr*decay**3
        elif "backbone.blocks.4" in name: lr=base_lr*decay**2
        elif "backbone.blocks.5" in name: lr=base_lr*decay**1
        else: lr=base_lr
...
def load_born_again(model, cp, dev):
    ...
    log.info("  Born-again from %s (val %.2f%%)",cp,100*ck.get("val_acc",0))
...
def main():
    ...
    log.info("★★★ EfficientNetV2-S @ 384 + SWA ★★★")
    ...
    log.info("KD α=%.2f τ=%.1f | born-again from v24 (gen 2)",alpha,tau)
    ...
    for fold,(tri,vai) in enumerate(skf.split(keys,labels)):
        ...
        ba=V24_DIR/f"best_fold{fold}.pt"
        if ba.exists():
            load_born_again(model,ba,dev)
        else:
            ba2=V12_DIR/f"best_fold{fold}.pt"
            if ba2.exists(): load_born_again(model,ba2,dev)
            else: log.info("  No born-again, using ImageNet pretrained")
        ...
        pg=get_llrd_params_effnet(model,CFG["stage2_lr"],decay=CFG["llrd_decay"],wd=CFG["weight_decay"])
