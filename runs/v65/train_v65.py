#!/usr/bin/env python3
"""
v65: CONTROL experiment — 3ch RTM @ 224 + spectral threshold -90dB + born-again from v11 +
8-teacher KD + SWA + FIXED LLRD + AMP. Same as v57 single-fold but with threshold + fixes.
Baseline to compare against v63 (6ch RTM+MTI) and v64 (3ch MTI-only).
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v65")
...
INPUT_MODE = "rtm"  # 3ch RTM with threshold (control)
TARGET_FOLD = 0
...
def main():
    ...
    log.info("Input mode: %s | Spectral threshold: %.0f dB | Fold: %d (CONTROL)", INPUT_MODE, SPECTRAL_THRESHOLD, TARGET_FOLD)
    ...
    log.info("KD α=%.2f τ=%.1f | SWA %d ep | 8-TEACHER KD | CONTROL (RTM+threshold)",alpha,tau,CFG["swa_epochs"])

    skf=StratifiedKFold(n_splits=CFG["n_folds"],shuffle=True,random_state=CFG["seed"])
    for fold,(tri,vai) in enumerate(skf.split(keys,labels)):
        if fold == TARGET_FOLD: break
    log.info("="*64); log.info("FOLD %d (single fold mode — CONTROL)",TARGET_FOLD); log.info("="*64)
