#!/usr/bin/env python3
"""
v59: ConvNeXtV2-Tiny @ 448 + 8-Teacher Ensemble KD + SWA. Born-again from v25. Uses ALL 8 models as teachers for ~91%+ accurate soft labels.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v59")
V25_BA_DIR = Path("/data/slr/checkpoints_v25")
...
CFG = {
    ...
    "max_frames": 48, "img_size": 448, "n_classes": 126,
    "channel_means": [-64.3217, -62.9119, -63.9254],
    "channel_stds": [12.5721, 11.6620, 11.7259],
    "n_folds": 5, "batch_size": 20, "num_workers": 2,
    "stage1_epochs": 5, "stage1_lr": 2e-4,
    "stage2_epochs": 120, "stage2_lr": 4e-5,
...
def precompute_teacher_labels(keys, cache, configs, dev):
    log.info("Pre-computing 8-teacher ensemble soft labels (per-teacher resolution) …")
    n_samples = len(keys)
    ...
    if n_teachers == 0:
        log.warning("No teachers produced soft labels; using uniform distribution")
...
def main():
    ...
    log.info("Student: ConvNeXtV2-Tiny @ %s×%s", CFG["img_size"], CFG["img_size"])
    ...
    log.info("KD α=%.2f τ=%.1f | SWA %d ep @ lr=%.1e | NO PL | 8-TEACHER ENSEMBLE KD",alpha,tau,CFG["swa_epochs"],CFG["swa_lr"])
    ...
    for fold,(tri,vai) in enumerate(skf.split(keys,labels)):
        ...
        ba=V25_BA_DIR/f"best_fold{fold}.pt"
        if ba.exists():
