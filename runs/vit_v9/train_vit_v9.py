...
ViT v9: CAFormer-S18 born-again from vit_v8 + cross-architecture Knowledge Distillation
(ConvNeXtV2 teachers teach CAFormer)

Key changes from vit_v8:
    1. Born-again from vit_v8 checkpoints (direct state_dict load)
    2. Cross-architecture teachers: v10 ConvNeXtV2 (5 folds) + v7 ConvNeXtGeM (5 folds)
    3. Stronger pseudo-labels: v10 (5 folds) + vit_v8 (5 folds), threshold 0.60
    4. Gentler training: Stage 1 = 5 epochs, Stage 2 = 150 epochs, base LR 5e-5
    5. LLRD decay 0.75, patience 40

Usage:
    conda run -n slr python /data/slr/train_vit_v9.py
"""
...
OUT_DIR = Path("/data/slr/checkpoints_vit_v9")
VIT_V8_DIR = Path("/data/slr/checkpoints_vit_v8")
V10_DIR = Path("/data/slr/checkpoints_v10")
...
class RadarDataset(Dataset):
    ...
    def _apply_aug(self, x):
        ...
        if np.random.random() < CFG["circ_shift_prob"]:
            shift = np.random.randint(-CFG["circ_shift_max"], CFG["circ_shift_max"] + 1)
...
class ConvNeXtGeM(nn.Module):
    def __init__(self, model_name, n_classes=126, pretrained=False, drop_path_rate=0.0):
        super().__init__()
        self.backbone = timm.create_model(
        ...
        self.gem = GeM(p=3.0)
        self.head = MultiSampleDropout(n_feat, n_classes, n_samples=5, drop_rate=0.3)
...
def get_llrd_params(model, base_lr, decay=0.75, wd=0.05):
    ...
    for name, p in model.named_parameters():
        ...
        if "backbone.stem" in name or "backbone.stages_0" in name:
            lr = base_lr * decay ** 4
...
def generate_pseudo_labels(eval_keys, eval_cache, pl_configs, dev, threshold=0.60):
    log.info("Generating pseudo-labels (threshold=%.2f) …", threshold)
    ds = RadarDataset(eval_keys, [0] * len(eval_keys), eval_cache, augment=False, img_size=CFG["img_size"])
    loader = DataLoader(ds, batch_size=96, shuffle=False, num_workers=CFG["num_workers"], pin_memory=True)
    teachers = _load_teachers_from_config(pl_configs, dev)
...
def train_epoch(model, loader, optim, sched, crit, dev, alpha, tau, use_mix=True):
    model.train()
    ...
    for x, y, soft in tqdm(loader, desc="  train", leave=False):
        ...
        correct += (lam * preds.eq(ya).float() + (1 - lam) * preds.eq(yb).float()).sum().item()
        n += bs
...
def main():
    ...
    pl_keys, pl_labels, pl_soft = generate_pseudo_labels(
        eval_keys, eval_cache, PL_CONFIGS, dev,
    ...
    alpha = CFG["kd_alpha"]
    tau = CFG["kd_tau"]
    log.info("KD: α=%.2f  τ=%.1f  | PL: %d extra samples", alpha, tau, len(pl_keys))

    probe = MultiScaleCAFormer(
    ...
    log.info("MultiScaleCAFormer: %.2fM params", n_params / 1e6)
    del probe
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(keys, labels)):
        ...
        va_ds = RadarDataset(va_keys, va_labels, combined_cache, augment=False, img_size=CFG["img_size"])
        ...
        model = MultiScaleCAFormer(
            CFG["model_name"], CFG["n_classes"], pretrained=True,
            drop_path_rate=CFG["drop_path_rate"], proj_dim=CFG["proj_dim"],
            drop_rate=CFG["drop_rate"], ms_samples=CFG["ms_dropout_samples"],
        ).to(dev)

        born_again_path = VIT_V8_DIR / f"best_fold{fold}.pt"
        if born_again_path.exists():
            load_born_again(model, born_again_path, dev)
        else:
            log.warning("  No born-again checkpoint at %s, using ImageNet init", born_again_path)

        crit = nn.CrossEntropyLoss(label_smoothing=CFG["label_smoothing"])
        best_acc = 0.0
        ckpt_path = out / f"best_fold{fold}.pt"

        log.info("── Stage 1: head-only warmup (%d epochs, no mix) ──", CFG["stage1_epochs"])

        for p in model.backbone.parameters():
            p.requires_grad = False

        s1_params = [p for p in model.parameters() if p.requires_grad]
        opt = AdamW(s1_params, lr=CFG["stage1_lr"], weight_decay=CFG["weight_decay"])
        ...
        for epoch in range(1, CFG["stage1_epochs"] + 1):
            t0 = time.time()
            tr_loss, tr_acc = train_epoch(model, tr_loader, opt, sched, crit, dev, alpha, tau, use_mix=False)
            va_loss, va_acc = eval_epoch(model, va_loader, crit, dev)
            dt = time.time() - t0
            log.info(
                "  S1 Ep %2d/%d  train %.4f / %.1f%%  val %.4f / %.1f%%    %.0fs",
                epoch, CFG["stage1_epochs"], tr_loss, 100 * tr_acc, va_loss, 100 * va_acc, dt,
            )
            if va_acc > best_acc:
                best_acc = va_acc
                torch.save({
        ...
        opt = AdamW(param_groups, lr=CFG["stage2_lr"], weight_decay=CFG["weight_decay"])
        ...
        for epoch in range(1, CFG["stage2_epochs"] + 1):
            t0 = time.time()
            tr_loss, tr_acc = train_epoch(model, tr_loader, opt, sched, crit, dev, alpha, tau, use_mix=True)
            va_loss, va_acc = eval_epoch(model, va_loader, crit, dev)
            dt = time.time() - t0
    ...
    log.info("Loading vit_v9 checkpoints for ensemble …")
    models_v9 = []
    for fold in range(CFG["n_folds"]):
        m = MultiScaleCAFormer(
            CFG["model_name"], CFG["n_classes"], pretrained=False,
    ...
    probs_no = predict_ensemble_tta(models_v9, eval_loader, dev, use_tta=False)
    preds_no = probs_no.argmax(dim=1).numpy()
    _write_submission(eval_keys, preds_no, out / "submission_no_tta.csv")
    log.info("Submission (no TTA) → %s  (%d rows)", out / "submission_no_tta.csv", len(preds_no))

    log.info("Running 10-way TTA × 5 folds …")
    probs_v9 = predict_ensemble_tta(models_v9, eval_loader, dev, use_tta=True)
    ...
    log.info("TTA changed %d / %d (%.1f%%)", changed, len(preds_v9), 100 * changed / len(preds_v9))

    torch.save({"keys": eval_keys, "probs": probs_v9}, out / "eval_probs_tta.pt")
    log.info("Saved vit_v9 TTA probs → %s", out / "eval_probs_tta.pt")
