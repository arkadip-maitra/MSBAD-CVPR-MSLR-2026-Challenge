#!/usr/bin/env python3
"""
v115: 5-fold CV — ConvNeXtV2-Tiny with 6-channel input (3 RTM + 3 FFT magnitude).
WEAK augmentation, from ImageNet pretrain (stem adapted to 6 channels).
Hypothesis: FFT adds Doppler/velocity info that helps distinguish similar signs.
600 epochs, NO early stopping.
Usage: python train_5fold_v115.py --fold 0 --gpu 0
"""
...
CFG = {
    ...
    "stage2_epochs": 600, "stage2_lr": 5e-5, "n_input_channels": 6,
...
def _add_fft_channels(x):
    """Add FFT magnitude channels. Input: (3, T, R), Output: (6, T, R)"""
    fft_mag = np.abs(np.fft.fft(x, axis=1)).astype(np.float32)
    fft_mag = np.log1p(fft_mag)
    for c in range(3):
        mu = fft_mag[c].mean(); sd = fft_mag[c].std() + 1e-8
        fft_mag[c] = (fft_mag[c] - mu) / sd
    return np.concatenate([x, fft_mag], axis=0)
...
class MultiScaleConvNeXtV2(nn.Module):
    def __init__(self, model_name, n_classes=126, pretrained=True,
                 drop_path_rate=0.1, proj_dim=512, drop_rate=0.2, ms_samples=5, in_chans=3):
        super().__init__()
...
def main():
    ...
    log.info("5-fold CV | %s | Fold %d | 6ch (RTM+FFT) | WEAK aug | 600 ep", VERSION, TARGET_FOLD)
    ...
    model=MultiScaleConvNeXtV2(CFG["model_name"],CFG["n_classes"],pretrained=True,
                               ...
                               in_chans=CFG["n_input_channels"]).to(dev)

    log.info("  Training from ImageNet pretrain (6-ch adapted, no born-again)")
    ...
    m=MultiScaleConvNeXtV2(CFG["model_name"],CFG["n_classes"],pretrained=False,drop_path_rate=0.0,
                           proj_dim=CFG["proj_dim"],drop_rate=CFG["drop_rate"],ms_samples=CFG["ms_dropout_samples"],
                           in_chans=CFG["n_input_channels"]).to(dev)
