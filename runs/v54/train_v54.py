#!/usr/bin/env python3
"""
v54: DINOv2 ViT-B/14 @ 224×224 + SWA
Completely different backbone: ViT with self-supervised DINOv2 pretraining (86M params).
Features trained on 142M images with masked image modeling — much richer than ImageNet supervised.
No born-again (different architecture), KD from v11+vit_v9.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v54")
...
CFG = {
    ...
    "stage1_epochs": 3, "stage1_lr": 5e-4,
    "stage2_epochs": 120, "stage2_lr": 1e-4,
    "swa_epochs": 15, "swa_lr": 2e-6,
    ...
    "drop_path_rate": 0.1, "drop_rate": 0.3,
...
class DINOv2Classifier(nn.Module):
    def __init__(self, model_name, n_classes=126, pretrained=True,
                 drop_path_rate=0.1, drop_rate=0.3, ms_samples=5, img_size=224):
        super().__init__()
        ...
        self.head = MultiSampleDropout(embed_dim, n_classes, n_samples=ms_samples, drop_rate=drop_rate)
...
def get_llrd_params_vit(model, base_lr, decay=0.65, wd=0.05):
    groups={}
    n_blocks = len(model.backbone.blocks)
    for name,p in model.named_parameters():
        if not p.requires_grad: continue
        if any(s in name for s in ["patch_embed","cls_token","pos_embed"]):
...
def main():
    ...
    log.info("KD α=%.2f τ=%.1f | DINOv2 ViT-B | No born-again",alpha,tau)
    ...
    for fold,(tri,vai) in enumerate(skf.split(keys,labels)):
        ...
        model=DINOv2Classifier(CFG["model_name"],CFG["n_classes"],pretrained=True,
        ...
        pg=get_llrd_params_vit(model,CFG["stage2_lr"],decay=CFG["llrd_decay"],wd=CFG["weight_decay"])
    ...
    fms=[]
    for f in range(CFG["n_folds"]):
        m=DINOv2Classifier(CFG["model_name"],CFG["n_classes"],pretrained=False,
                           drop_path_rate=0.0,drop_rate=CFG["drop_rate"],
                           ms_samples=CFG["ms_dropout_samples"],img_size=CFG["img_size"]).to(dev)
