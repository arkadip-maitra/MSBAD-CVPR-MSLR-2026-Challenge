#!/usr/bin/env python3
"""
v93: ConvNext v1 (not v2) — same stage dims as v2, full born-again from v67.
Different normalization and pretraining (supervised ImageNet-22k, no FCMAE).
Provides genuine architectural diversity for ensembling.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v93")
...
def load_born_again(model, cp, dev):
    ...
    filtered_sd = {k: v for k, v in ck["model_state_dict"].items()
                   if k in model_keys and ck["model_state_dict"][k].shape == model.state_dict()[k].shape}
    model.load_state_dict(filtered_sd, strict=False)
    loaded = len(filtered_sd)
    total = len(model.state_dict())
    log.info("  Born-again ← %s (val %.2f%%, loaded %d/%d params)", cp.name, 100*ck.get("val_acc",0), loaded, total)
