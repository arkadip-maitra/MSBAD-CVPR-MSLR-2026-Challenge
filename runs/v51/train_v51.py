#!/usr/bin/env python3
"""
v51: CAFormer-S18 @ 288×288 + SWA. Fill CAFormer resolution gap.
Born-again from vit_v9 (224×224) + KD from v11 (ConvNeXtV2-Tiny teacher @ 224).
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v51")
...
def get_llrd_params(model, base_lr, decay=0.75, wd=0.05):
    """Layer-wise LR decay for timm CAFormer / MetaFormer: backbone.patch_embed, backbone.network.0…3."""
    groups={}
    for name,p in model.named_parameters():
        ...
        if name.startswith("backbone."):
            ...
            if rest.startswith("patch_embed"):
                lr=base_lr*decay**5
            elif rest.startswith("network.3."):
                lr=base_lr*decay**1
            elif rest.startswith("network.2."):
                lr=base_lr*decay**2
            elif rest.startswith("network.1."):
                lr=base_lr*decay**3
            elif rest.startswith("network.0."):
                lr=base_lr*decay**4
            elif rest.startswith("network."):
                lr=base_lr*decay**2
            else:
                lr=base_lr*decay**5
    ...
    log.info("LLRD (CAFormer): %d groups, LR [%.2e … %.2e]",len(r),min(lrs),max(lrs)); return r
...
def main():
    ...
    log.info("★★★ v51 CAFormer-S18 @ %d×%d + SWA | born-again vit_v9 | KD v11 ★★★", CFG["img_size"], CFG["img_size"])
    ...
    log.info("KD α=%.2f τ=%.1f | Teacher: v11 ConvNeXtV2-Tiny @ %d | Born-again: vit_v9",
             alpha,tau,CFG["teacher_img_size"])
