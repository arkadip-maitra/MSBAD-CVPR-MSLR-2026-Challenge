#!/usr/bin/env python3
"""
v55: ConvNeXtV2-Tiny @ 224 + Pseudo-Labeling + SWA
Uses 6-model ensemble to pseudo-label test data, expanding training by ~50%.
Born-again from v11, KD from vit_v9.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v55")
...
PSEUDO_LABEL_CONF = 0.70
PSEUDO_MODELS = {
    "v11": ("checkpoints_v11", "eval_probs_tta.pt"),
    "vit_v9": ("checkpoints_vit_v9", "eval_probs_tta.pt"),
    "v18": ("checkpoints_v18", "eval_probs_tta.pt"),
    "v25": ("checkpoints_v25", "eval_probs_tta.pt"),
    "v24": ("checkpoints_v24", "eval_probs_tta.pt"),
    "v38": ("checkpoints_v38", "eval_probs_tta.pt"),
}
...
CFG = {
    ...
    "stage2_epochs": 100, "stage2_lr": 4e-5,
    "swa_epochs": 15, "swa_lr": 4e-6,
...
def precompute_teacher_labels(keys, cache, configs, dev):
    ...
    loader=DataLoader(ds,batch_size=96,shuffle=False,num_workers=2,pin_memory=True)
    ts=_load_teachers(configs,dev); log.info("  Loaded %d teachers",len(ts)); nt=len(ts)
    ap=[]
    with torch.no_grad():
...
def compute_pseudo_labels(eval_cache, eval_keys, conf_threshold):
    """Load ensemble predictions for test data, filter by confidence."""
    log.info("Computing pseudo-labels from 6-model ensemble (threshold=%.2f)…", conf_threshold)
    all_probs = []
    for name, (ckpt_dir, prob_file) in PSEUDO_MODELS.items():
        p = Path(f"/data/slr/{ckpt_dir}/{prob_file}")
        if not p.exists():
            log.warning("  Missing: %s", p); continue
        d = torch.load(p, map_location="cpu", weights_only=True)
        all_probs.append(d["probs"])
        log.info("  Loaded %s: %s", name, d["probs"].shape)

    geo_mean = torch.exp(sum(torch.log(p + 1e-8) for p in all_probs) / len(all_probs))
    geo_mean = geo_mean / geo_mean.sum(dim=1, keepdim=True)
    pred_labels = geo_mean.argmax(1)
    pred_conf = geo_mean.max(1).values
    log.info("  Confidence stats: mean=%.3f, median=%.3f, max=%.3f, min=%.3f",
             pred_conf.mean(), pred_conf.median(), pred_conf.max(), pred_conf.min())

    above_thresh = pred_conf >= conf_threshold
    n_above = above_thresh.sum().item()
    log.info("  Pseudo-labeled: %d / %d (%.1f%%) above threshold %.2f",
             n_above, len(eval_keys), 100*n_above/len(eval_keys), conf_threshold)

    pseudo_keys, pseudo_labels, pseudo_soft = [], [], {}
    for i, key in enumerate(eval_keys):
        if above_thresh[i]:
            pseudo_keys.append(key)
            pseudo_labels.append(pred_labels[i].item())
            pseudo_soft[key] = geo_mean[i]

    log.info("  Class distribution: %d unique classes in pseudo-labels",
             len(set(pseudo_labels)))
    return pseudo_keys, pseudo_labels, pseudo_soft
...
def main():
    ...
    log.info("★★★ PSEUDO-LABELING + ConvNeXtV2 @ 224 ★★★")
    ...
    pseudo_keys, pseudo_labels, pseudo_soft = compute_pseudo_labels(eval_cache, eval_keys, PSEUDO_LABEL_CONF)
    all_cache = {**train_cache, **eval_cache}

    json.dump(CFG,open(out/"config.json","w"),indent=2)
    soft_labels=precompute_teacher_labels(keys,train_cache,TEACHER_CONFIGS,dev)
    soft_labels.update(pseudo_soft)
    alpha,tau=CFG["kd_alpha"],CFG["kd_tau"]
    log.info("KD α=%.2f τ=%.1f | born-again v11 | +%d pseudo-labeled", alpha, tau, len(pseudo_keys))
    ...
    for fold,(tri,vai) in enumerate(skf.split(keys,labels)):
        ...
        aug_trk = trk + pseudo_keys
        aug_trl = trl + pseudo_labels
        log.info("  Train: %d (orig) + %d (pseudo) = %d | Val: %d",
                 len(trk), len(pseudo_keys), len(aug_trk), len(vak))

        tds=RadarDataset(aug_trk,aug_trl,all_cache,augment=True,soft_labels=soft_labels,
        ...
        for swa_ep in range(1,CFG["swa_epochs"]+1):
            ...
            log.info("  SWA Ep %2d/%d  loss=%.4f  swa_val=%.1f%%  %.0fs",swa_ep,CFG["swa_epochs"],trl_,100*sva,time.time()-t0)
