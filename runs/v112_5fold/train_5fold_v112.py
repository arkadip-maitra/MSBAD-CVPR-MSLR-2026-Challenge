#!/usr/bin/env python3
"""
v112: 5-fold CV — EfficientNetV2-S (different architecture) with WEAK augmentation.
FROM ImageNet pretrain (no born-again). 600 epochs, NO early stopping.
Tests if architecture diversity helps break the ConvNeXtV2 plateau.
Usage: python train_5fold_v112.py --fold 0 --gpu 0
"""
...
def get_llrd_params(model, base_lr, decay=0.75, wd=0.05):
    ...
    for name,p in model.named_parameters():
        ...
        if "backbone.conv_stem" in name or "backbone.bn1" in name:
            lr=base_lr*decay**6
        elif "backbone.blocks.0" in name or "backbone.blocks.1" in name:
            lr=base_lr*decay**5
        elif "backbone.blocks.2" in name or "backbone.blocks.3" in name:
            lr=base_lr*decay**4
        elif "backbone.blocks.4" in name:
            lr=base_lr*decay**3
        elif "backbone.blocks.5" in name:
            lr=base_lr*decay**2
        elif "backbone.blocks.6" in name:
            lr=base_lr*decay**1
        else: lr=base_lr
...
def main():
    ...
    log.info("5-fold CV | %s | Fold %d | EfficientNetV2-S | WEAK aug | 600 ep", VERSION, TARGET_FOLD)
    ...
    log.info("  Training from ImageNet pretrain (no born-again)")
