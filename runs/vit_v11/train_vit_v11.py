#!/usr/bin/env python3
"""
vit_v11: CAFormer-S18 + SAM (Sharpness-Aware Minimization)
Born-again from vit_v9, NO pseudo-labels, cross-arch KD from v11 + v12

Key changes vs vit_v10:
  ...
  3. Born-again from vit_v9 (LB-proven, not the overfit vit_v10)
"""
...
OUT_DIR = Path("/data/slr/checkpoints_vit_v11")
...
def main():
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(keys, labels)):
        ...
        if born_again_path.exists():
            ...
            log.warning("  No born-again checkpoint at %s", born_again_path)
    ...
    log.info("Loading vit_v11 checkpoints for ensemble …")
    fold_models = []
    for fold in range(CFG["n_folds"]):
    ...
    log.info("Saved vit_v11 TTA probs → %s", out / "eval_probs_tta.pt")
