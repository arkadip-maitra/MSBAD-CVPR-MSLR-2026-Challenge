#!/usr/bin/env python3
"""
v80: Physics-guided motion weighting — emphasize range bins with high temporal
variance (gesture motion at bins 80-130) while suppressing static body reflections
(bins 0-10) and noise. Born-again from v66.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v80")
...
class RadarDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Physics-guided motion weighting: amplify range bins with high temporal variance
        # This emphasizes gesture motion (bins 80-130) over static body (bins 0-10) and noise
        tvar = x.var(axis=1, keepdims=True)  # (3, 1, R) — temporal variance per range bin
        tvar_max = tvar.max(axis=2, keepdims=True) + 1e-8  # (3, 1, 1)
        motion_weight = np.sqrt(tvar / tvar_max)  # sqrt to soften, range [0, 1]
        x = x * (0.3 + 0.7 * motion_weight)  # blend: 30% original + 70% motion-weighted
...
def main():
    ...
    log.info("Motion-weighted range bins | born-again v66 | Fold %d", TARGET_FOLD)
    ...
    log.info("="*64); log.info("FOLD %d — Motion-weighted range bins",TARGET_FOLD); log.info("="*64)
