...
ViT v3: Swin-Tiny (ImageNet-22k) + two-stage training + GeM + Multi-Sample Dropout.

Fixes over ViT v2 (54%):
  - Swin-Tiny replaces DeiT3-Small: local window attention = CNN-like inductive
    biases, relative position bias (no absolute pos embed issues), hierarchical
  - NO EMA (v2's EMA+OneCycleLR kept shadow at 0.7% for 10+ epochs)
  - Two-stage training: stage 1 trains head only (backbone frozen) to build a
    good classifier on pretrained features; stage 2 unfreezes everything with
    LLRD for careful fine-tuning
  - GeM pooling + Multi-Sample Dropout (from v4 CNN)
  - Repeat padding instead of zero padding
  - Stochastic depth (drop_path=0.2) for regularization

Usage:
    conda run -n slr python /data/slr/train_vit_v3.py
"""
...
# ---------------------------------------------------------------------------
OUT_DIR = Path("/data/slr/checkpoints_vit_v3")
...
class RadarDataset(Dataset):
    ...
    def _pad_or_crop(x, max_t, random_crop=False):
        """Repeat-pad short sequences; crop long ones."""
        t = x.shape[1]
        if t > max_t:
            s = (
...
class SwinGeM(nn.Module):
    """Swin-Tiny backbone + GeM pooling + Multi-Sample Dropout head."""
    ...
    def __init__(self, n_classes=126, pretrained=True):
        ...
        self.n_feat = self.backbone.num_features  # 768
    ...
    def freeze_backbone(self):
        ...
        log.info("Backbone frozen (%d params fixed)",
                 sum(p.numel() for p in self.backbone.parameters()))

    def unfreeze_backbone(self):
        for param in self.backbone.parameters():
            param.requires_grad_(True)
        log.info("Backbone unfrozen (%d params trainable)",
                 sum(p.numel() for p in self.parameters() if p.requires_grad))

    def forward(self, x):
        feats = self.backbone.forward_features(x)   # (B, 7, 7, 768) NHWC
        feats = feats.permute(0, 3, 1, 2).contiguous()  # → NCHW
        pooled = self.gem(feats).flatten(1)           # (B, 768)
        return self.head(pooled)


# ===========================================================================
#  LLRD — stage-wise for Swin
# ===========================================================================


def get_swin_param_groups(model, base_lr, stage_decay, wd):
    """Assign per-stage LR: patch_embed < layers.0 < ... < layers.3 < head."""
    groups = {}
...
def main():
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(keys, labels)):
        ...
        # ==== STAGE 2: Full fine-tuning with LLRD ==========================
        log.info("── Stage 2: full fine-tuning (%d epochs) ──",
                 CFG["stage2_epochs"])
        ...
        for epoch in range(1, CFG["stage2_epochs"] + 1):
            ...
            if va_acc > best_acc:
                ...
                if patience >= CFG["early_stopping_patience"]:
                    log.info("  Early stopping at stage 2 epoch %d", epoch)
                    break
    ...
    for fold in range(CFG["n_folds"]):
        m = SwinGeM(n_classes=CFG["n_classes"], pretrained=False).to(dev)
        ckpt = torch.load(
