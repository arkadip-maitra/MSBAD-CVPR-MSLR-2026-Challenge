#!/usr/bin/env python3
"""
v92: Proper linear-domain MTI + original signal as 6-channel input.
Channels 0-2: original 3-antenna RTMs (z-score normalized)
Channels 3-5: linear-domain MTI of 3 antennas (remove static, re-normalize)
Born-again from v67. Stem conv layer reinitialized for 6 channels.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v92")
V67_DIR = Path("/data/slr/checkpoints_v67")
...
class RadarDataset(Dataset):
    ...
    def _apply_aug(self, x):
        ...
        if np.random.random()<CFG["channel_shuffle_prob"]:
            ...
            x = x[np.concatenate([perm, perm + 3])]
    ...
    def __getitem__(self, idx):
        raw=self.cache[self.keys[idx]].copy()
        x_orig=(raw-_MEANS)/_STDS
        # Linear-domain MTI: convert dB→linear, subtract temporal mean, convert back to dB
        x_lin = np.power(10.0, raw / 10.0)
        x_lin_mean = x_lin.mean(axis=1, keepdims=True)
        x_mti_lin = x_lin - x_lin_mean
        eps = 1e-10
        x_mti_db = 10.0 * np.log10(np.maximum(np.abs(x_mti_lin), eps))
        # Z-score normalize the MTI channels independently
        for ch in range(3):
            mu = x_mti_db[ch].mean(); sd = x_mti_db[ch].std() + 1e-6
            x_mti_db[ch] = (x_mti_db[ch] - mu) / sd
        # Stack: 6 channels = [orig_0, orig_1, orig_2, mti_0, mti_1, mti_2]
        x = np.concatenate([x_orig, x_mti_db.astype(np.float32)], axis=0)
...
class MultiScaleConvNeXtV2(nn.Module):
    def __init__(self, model_name, n_classes=126, pretrained=True,
                 drop_path_rate=0.2, proj_dim=512, drop_rate=0.3, ms_samples=5, in_channels=3):
        super().__init__()
...
def load_born_again(model, cp, dev):
    ...
    # Handle 3→6 channel stem: find stem conv key and expand channels
    stem_key = None
    for k in filtered_sd:
        if "stem" in k and "weight" in k and filtered_sd[k].ndim == 4 and filtered_sd[k].shape[1] == 3:
            if k in model_keys and model.state_dict()[k].shape[1] == 6:
                stem_key = k; break
    if stem_key is not None:
        w3 = filtered_sd[stem_key]
        w6 = torch.cat([w3, w3 * 0.1], dim=1)  # MTI channels start with small weights
        filtered_sd[stem_key] = w6
        log.info("  Stem expanded: %s → %s", w3.shape, w6.shape)
    model.load_state_dict(filtered_sd, strict=False)
    skipped = len(ck["model_state_dict"]) - len(filtered_sd)
    log.info("  Born-again ← %s (val %.2f%%, skipped %d extra keys)", cp.name, 100*ck.get("val_acc",0), skipped)
...
def _tta(x):
    ...
    if nc == 6:
        augs.append(x[:,[1,2,0,4,5,3],:,:])
        augs.append(x[:,[2,0,1,5,3,4],:,:])
    else:
        augs.append(x[:,[1,2,0],:,:])
        augs.append(x[:,[2,0,1],:,:])
    augs.extend([x+.05*torch.randn_like(x), x+.08*torch.randn_like(x), x*.9, x*1.1])
    return augs
@torch.no_grad()
...
def main():
    ...
    log.info("NO KD | NO threshold | AMP | LLRD fixed | Fold %d", TARGET_FOLD)
    ...
    log.info("="*64); log.info("FOLD %d — CLEAN BASELINE (no KD, no threshold)",TARGET_FOLD); log.info("="*64)
    ...
    model=MultiScaleConvNeXtV2(CFG["model_name"],CFG["n_classes"],pretrained=True,
                               ...
                               in_channels=CFG["in_channels"]).to(dev)
    ba=V67_DIR/f"best_fold{TARGET_FOLD}.pt"
