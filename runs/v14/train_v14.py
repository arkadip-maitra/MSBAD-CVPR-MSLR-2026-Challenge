#!/usr/bin/env python3
"""
v14: ConvNeXtV2-Tiny + SAM (Sharpness-Aware Minimization)
...
Key changes vs v13:
  ...
  3. Born-again from v11 (LB-proven, not the overfit v13)
"""
...
OUT_DIR = Path("/data/slr/checkpoints_v14")
...
def main():
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(keys, labels)):
        ...
        # ── Stage 1: head-only warmup (regular AdamW, no SAM) ──
    ...
    log.info("Loading v14 checkpoints for ensemble …")
    fold_models = []
    for fold in range(CFG["n_folds"]):
    ...
    log.info("Saved v14 TTA probs → %s", out / "eval_probs_tta.pt")
