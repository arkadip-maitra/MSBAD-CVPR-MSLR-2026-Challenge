...
ViT v10: CAFormer-S18 born-again from vit_v9 + triple cross-architecture KD
(ConvNeXtV2 + EfficientNetV2 teach CAFormer)

Usage:
    conda run -n slr python /data/slr/train_vit_v10.py
"""
...
OUT_DIR = Path("/data/slr/checkpoints_vit_v10")
...
def generate_pseudo_labels(eval_keys, eval_cache, pl_configs, dev, threshold=0.50):
...
def main():
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(keys, labels)):
        ...
        born_again_path = VIT_V9_DIR / f"best_fold{fold}.pt"
    ...
    log.info("Loading vit_v10 checkpoints for ensemble …")
    models_v10 = []
    for fold in range(CFG["n_folds"]):
    ...
    log.info("TTA changed %d / %d (%.1f%%)", changed, len(preds_v10), 100 * changed / len(preds_v10))

    torch.save({"keys": eval_keys, "probs": probs_v10}, out / "eval_probs_tta.pt")
    log.info("Saved vit_v10 TTA probs → %s", out / "eval_probs_tta.pt")
