#!/usr/bin/env python3
"""
v56: ConvNeXtV2-Tiny @ NATIVE (64×256) + SWA
Native aspect ratio: raw data is (3, ~27, 256). Current approach stretches time 8.4x.
This uses (64, 256) — only 2.4x stretch — preserving temporal-spatial structure.
Born-again from v11 (conv weights are resolution-agnostic, GeM pools to fixed size).
KD from vit_v9.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v56")
...
CFG = {
    ...
    "n_folds": 5, "batch_size": 64, "num_workers": 2,
    ...
    "time_mask_max": 10, "range_mask_max": 30, "noise_std": 0.1,
    "time_warp_lo": 0.8, "time_warp_hi": 1.2,
    "time_reverse_prob": 0.5, "channel_shuffle_prob": 0.2, "channel_drop_prob": 0.1,
    "random_erase_prob": 0.20, "circ_shift_prob": 0.3, "circ_shift_max": 10,
...
def main():
    ...
    log.info("★★★ NATIVE RESOLUTION: %s — 2.4x temporal stretch (vs 8.4x) ★★★", CFG["img_size"])
    ...
    json.dump(CFG,open(out/"config.json","w"),indent=2,default=str)
    soft_labels=precompute_teacher_labels(keys,train_cache,TEACHER_CONFIGS,dev)
    alpha,tau=CFG["kd_alpha"],CFG["kd_tau"]
    log.info("KD α=%.2f τ=%.1f | born-again from v11 | native (64,256)",alpha,tau)
