#!/usr/bin/env python3
"""
v67: CLIP text alignment as auxiliary loss. Born-again from v11.
Inspired by TRACE (99.3% radar-only): uses CLIP text encoder to encode class labels,
then trains radar encoder to produce features aligned with text embeddings.
Loss = α * CE + (1-α) * CLIP_CE where CLIP_CE uses cosine similarity logits.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v67")
...
LABEL_TRANSLATIONS = {
    ...
    "U":"letter U","V":"letter V","W":"letter W","X":"letter X","Y":"letter Y","Z":"letter Z",
    "AIUTARE":"help","ALTRO":"other","AMBULANZA":"ambulance","ANSIA":"anxiety",
    "BATTITO_CARDIACO":"heartbeat","BOCCA":"mouth","BRACCIO":"arm","BRIVIDO":"shiver",
    "CALORE":"heat","CASA":"home","CHIAMARE":"to call","CHIEDERE":"to ask",
    "COLLO":"neck","COME":"how","COMPRARE":"to buy","COMUNICARE":"to communicate",
    "CONTAGIOSO":"contagious","CORONAVIRUS":"coronavirus","CUORE":"heart","CURARE":"to cure",
    "DIFFICOLTA":"difficulty","DISINFETTARE":"to disinfect","DOLORE":"pain",
    "DOMANI":"tomorrow","DOTTORE":"doctor","DOVE":"where","DOVERE":"must",
    "ELICOTTERO":"helicopter","ESSERCI":"to be there","FARMACIA":"pharmacy",
    "FEGATO":"liver","FIGLIO":"child","FRONTE2":"forehead","FUMARE":"to smoke",
    "GAMBE":"legs","GIORNO":"day","GOLA":"throat","GRAVE":"serious",
    "IERI":"yesterday","INGUINE":"groin","INIEZIONE":"injection","IO":"I myself",
    "LEGGERO":"light mild","LETTO":"bed","MALATTIA":"disease","MAMMA":"mother",
    "MANGIARE":"to eat","MANO":"hand","MASCHERINA":"face mask","MATTINA":"morning",
    "MEDICINE":"medicine","MEZZOGIORNO":"noon","MOLTO":"very much","MUSCOLO":"muscle",
    "NASO":"nose","NAUSEA":"nausea","NO":"no","NOME":"name",
    "OCCHIO":"eye","ORECCHIO":"ear","OSPEDALE":"hospital",
    "PALLORE":"pallor","PAPA":"father","PERCHE":"why","PERSONA":"person",
    "PETTO":"chest","PIEDI":"feet","PILLOLA":"pill","POSITIVO":"positive",
    "POTERE":"can","PRIMA":"before","PROBLEMA":"problem","PRONTO_SOCCORSO":"emergency room",
    "QUANDO":"when","RAFFREDDORE":"cold flu","RESPIRARE":"to breathe",
    "RESTARE":"to stay","RUMORE":"noise","SCHIENA":"back","SCIROPPO":"syrup",
    "SE":"if","SERA":"evening","SI":"yes","SINTOMI":"symptoms",
    "SMETTERE":"to stop","SORDO":"deaf","SPALLE":"shoulders","STANCHEZZA":"tiredness",
    "STOMACO":"stomach","SUBITO":"immediately","SUDORE":"sweat",
    "TEMPO":"time","TERMOMETRO":"thermometer","TESTA":"head",
    "TOSSE":"cough","TU":"you","VACCINO":"vaccine","VERTIGINI":"dizziness",
    "VISTA":"vision sight","VOMITO":"vomit",
}
...
CFG = {
    ...
    "stage1_epochs": 5, "stage1_lr": 2e-4,
    "stage2_epochs": 150, "stage2_lr": 4e-5,
    "swa_epochs": 20, "swa_lr": 4e-6,
...
class CLIPAlignedConvNeXtV2(nn.Module):
    def __init__(self, model_name, n_classes=126, pretrained=True,
                 drop_path_rate=0.2, proj_dim=512, drop_rate=0.3, ms_samples=5, clip_dim=512):
        super().__init__()
    ...
    def forward(self, x, text_feats=None):
        features = self.get_features(x)
        logits = self.head(features)
        if text_feats is not None:
            clip_emb = F.normalize(self.clip_proj(features), dim=-1)
            clip_logits = clip_emb @ text_feats.T * self.log_temp.exp()
            return logits, clip_logits
        return logits

def compute_text_embeddings(idx_to_cls, dev):
    ...
    prompts = []
    for i in range(126):
        raw = idx_to_cls[i].split("_", 1)[1] if "_" in idx_to_cls[i] else idx_to_cls[i]
        eng = LABEL_TRANSLATIONS.get(raw, raw.lower().replace("_", " "))
        prompts.append(f"a sign language gesture meaning {eng}")
    ...
    mask = ~torch.eye(126, device=dev).bool()
    log.info("CLIP text embeddings: mean_sim=%.3f, max_sim=%.3f, min_sim=%.3f",
             sim[mask].mean().item(), sim[mask].max().item(), sim[mask].min().item())
    del clip_model
...
def load_born_again(model, cp, dev):
    ...
    log.info("  Born-again ← %s (val %.2f%%) missing=%d unexpected=%d",
             cp.name, 100*ck.get("val_acc",0), len(missing), len(unexpected))
...
def train_epoch(model, loader, opt, sched, crit, dev, scaler, text_feats, clip_alpha, use_mix=True):
    model.train(); tl,co,n=0.,0.,0
    for x,y in tqdm(loader,desc="  train",leave=False):
        x,y=x.to(dev,non_blocking=True),y.to(dev,non_blocking=True)
        if use_mix: xm,ya,yb,lam=mix_or_cut(x,y)
        else: xm,ya,yb,lam=x,y,y,1.0
        with torch.amp.autocast('cuda'):
            logits, clip_logits = model(xm, text_feats)
            ce = lam*crit(logits,ya)+(1-lam)*crit(logits,yb)
            clip_ce = lam*F.cross_entropy(clip_logits,ya)+(1-lam)*F.cross_entropy(clip_logits,yb)
            loss = (1-clip_alpha)*ce + clip_alpha*clip_ce
...
def main():
    ...
    log.info("CLIP text alignment (α=%.2f) | Fold %d", CLIP_ALPHA, TARGET_FOLD)
    ...
    text_feats = compute_text_embeddings(idx_to_cls, dev)

    skf=StratifiedKFold(n_splits=CFG["n_folds"],shuffle=True,random_state=CFG["seed"])
    for fold,(tri,vai) in enumerate(skf.split(keys,labels)):
        if fold == TARGET_FOLD: break
    log.info("="*64); log.info("FOLD %d — CLIP TEXT ALIGNMENT",TARGET_FOLD); log.info("="*64)
    ...
    model=CLIPAlignedConvNeXtV2(CFG["model_name"],CFG["n_classes"],pretrained=True,
                                drop_path_rate=CFG["drop_path_rate"],proj_dim=CFG["proj_dim"],
                                drop_rate=CFG["drop_rate"],ms_samples=CFG["ms_dropout_samples"],
                                clip_dim=CLIP_DIM).to(dev)
    ba=V11_DIR/f"best_fold{TARGET_FOLD}.pt"
    if ba.exists(): load_born_again(model,ba,dev)

    scaler = torch.amp.GradScaler('cuda')
    crit=nn.CrossEntropyLoss(label_smoothing=CFG["label_smoothing"])
    best_acc=0.0; ckpt=out/f"best_fold{TARGET_FOLD}.pt"

    log.info("── Stage 1: head + clip_proj warmup (%d ep) ──",CFG["stage1_epochs"])
    for p in model.backbone.parameters(): p.requires_grad=False
    for p in model.stage_gems.parameters(): p.requires_grad=False
    for p in model.stage_projs.parameters(): p.requires_grad=False
    for p in model.scale_attn.parameters(): p.requires_grad=False
    ...
    for ep in range(1,CFG["stage1_epochs"]+1):
        t0=time.time()
        trl_,tra_=train_epoch(model,tl,opt,sched,crit,dev,scaler,text_feats,CLIP_ALPHA,use_mix=False)
    ...
    for ep in range(1,CFG["stage2_epochs"]+1):
        t0=time.time()
        trl_,tra_=train_epoch(model,tl,opt,sched,crit,dev,scaler,text_feats,CLIP_ALPHA,use_mix=True)
        _,va_=eval_epoch(model,vl,crit,dev)
        log.info("  S2 Ep %3d/%d  loss=%.4f acc=%.1f%%  val=%.1f%%  lr=%.2e  temp=%.1f  %4.0fs",
                 ep,CFG["stage2_epochs"],trl_,100*tra_,100*va_,opt.param_groups[-1]["lr"],
                 model.log_temp.exp().item(),time.time()-t0)
        if va_>best_acc:
            best_acc=va_; pat=0
    ...
    for swa_ep in range(1,CFG["swa_epochs"]+1):
        t0=time.time()
        trl_,_=train_epoch(model,tl,swa_opt,None,crit,dev,scaler,text_feats,CLIP_ALPHA,use_mix=True)
    ...
    json.dump({"fold":TARGET_FOLD,"accuracy":best_acc},open(out/"cv_results.json","w"),indent=2)
    ...
    m=CLIPAlignedConvNeXtV2(CFG["model_name"],CFG["n_classes"],pretrained=False,drop_path_rate=0.0,
                            proj_dim=CFG["proj_dim"],drop_rate=CFG["drop_rate"],
                            ms_samples=CFG["ms_dropout_samples"],clip_dim=CLIP_DIM).to(dev)
    m.load_state_dict(torch.load(ckpt,map_location=dev,weights_only=True)["model_state_dict"])
    pn=predict_tta([m],el,dev,tta=False); _write_sub(eval_keys,pn.argmax(1).numpy(),out/"submission_no_tta.csv")
    pt=predict_tta([m],el,dev,tta=True); _write_sub(eval_keys,pt.argmax(1).numpy(),out/"submission_tta.csv")
