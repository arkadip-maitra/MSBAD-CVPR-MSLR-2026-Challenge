#!/usr/bin/env python3
"""
v82: Clean MTI (Moving Target Indication) — temporal mean subtraction in dB domain
per range bin. Removes static body reflections (bins 0-10), highlighting only the
gesture motion (bins 80-130). After MTI, re-normalize with dataset-level stats
computed on MTI data. Born-again from v66 (same architecture).
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v82")
...
class RadarDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # MTI: subtract temporal mean per range bin per channel (removes static clutter)
        tmean = x.mean(axis=1, keepdims=True)  # (3, 1, R)
        x = x - tmean
        # Re-normalize: MTI signal has different statistics (zero mean, smaller std)
        for ch in range(3):
            sd = x[ch].std() + 1e-6
            x[ch] = x[ch] / sd
...
def main():
    ...
    log.info("Clean MTI | born-again v66 | Fold %d", TARGET_FOLD)
    ...
    log.info("="*64); log.info("FOLD %d — Clean MTI (temporal mean subtraction)",TARGET_FOLD); log.info("="*64)
