#!/usr/bin/env python3
"""
v76: Self-training with pseudo-labels — use v67 to predict on 9828 unlabeled samples,
add high-confidence predictions (>0.85) as extra training data. Nearly doubles training set.
Born-again from v66.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v76")
...
def main():
    ...
    log.info("SELF-TRAINING pseudo-labels | Born-again v66 | Fold %d", TARGET_FOLD)
    ...
    log.info("="*64); log.info("FOLD %d — SELF-TRAINING, born-again v66",TARGET_FOLD); log.info("="*64)
    ...
    if V67_PROBS.exists():
        ...
        log.info("  Pseudo-labels (>%.2f): %d / %d eval samples added",pseudo_thresh,len(pk),len(pkeys))
