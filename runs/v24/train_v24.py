#!/usr/bin/env python3
"""
v24: EfficientNetV2-S @ 384×384 + SWA + KD
Different backbone (MBConv+SE) at high resolution = maximum ensemble diversity.
Born-again from v12 (EfficientNetV2-S @224) + KD from v11+vit_v9.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v24")
...
def main():
    ...
    for fold,(tri,vai) in enumerate(skf.split(keys,labels)):
        ...
        ba=V12_DIR/f"best_fold{fold}.pt"
        if ba.exists():
            load_born_again(model,ba,dev)
        else:
            log.info("  No born-again, using ImageNet pretrained")
