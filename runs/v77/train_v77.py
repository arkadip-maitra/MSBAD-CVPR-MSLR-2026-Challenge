#!/usr/bin/env python3
"""
v77: No padding — resize directly from (3, T, 256) to (3, 224, 224).
Eliminates ALL padding artifacts (no tiling, no reflection, no zeros).
Each sample's temporal content fills the entire spatial extent.
Born-again from v66.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v77")
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            ...
            if np.random.random()<CFG["amp_scale_prob"]:
                x=x*np.random.uniform(CFG["amp_scale_lo"],CFG["amp_scale_hi"])
            if np.random.random()<0.3:
                f=np.random.uniform(CFG["time_warp_lo"],CFG["time_warp_hi"])
                nt=max(1,int(x.shape[1]*f))
                x=zoom(x,(1,nt/x.shape[1],1),order=1).astype(np.float32)
            if np.random.random()<0.5:
        ...
        if self.augment:
            for _ in range(2):
                if np.random.random()<0.5:
                    w=np.random.randint(1,CFG["time_mask_max"]+1); t0=np.random.randint(0,max(1,t.shape[1]-w)); t[:,t0:t0+w,:]=0.0
            for _ in range(2):
                if np.random.random()<0.5:
                    w=np.random.randint(1,CFG["range_mask_max"]+1); r0=np.random.randint(0,max(1,t.shape[2]-w)); t[:,:,r0:r0+w]=0.0
            t=_ERASER(t)
...
def main():
    ...
    log.info("NO PADDING direct resize | Born-again v66 | Fold %d", TARGET_FOLD)
    ...
    log.info("="*64); log.info("FOLD %d — NO PADDING, born-again v66",TARGET_FOLD); log.info("="*64)
