#!/usr/bin/env python3
"""
v78: Class-weighted focal loss targeting worst classes + self-training.
Combines: (1) focal loss for hard examples, (2) pseudo-labels, (3) born-again from v66.
The worst classes (U,E,V,X,Y,I,D,F,N,O,M) get extra attention via focal loss.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v78")
V66_DIR = Path("/data/slr/checkpoints_v66")
V67_PROBS = Path("/data/slr/checkpoints_v67/eval_probs_tta.pt")
...
def main():
    ...
    log.info("FOCAL LOSS + PSEUDO-LABELS | Born-again v66 | Fold %d", TARGET_FOLD)
    ...
    log.info("="*64); log.info("FOLD %d — FOCAL + PSEUDO, born-again v66",TARGET_FOLD); log.info("="*64)
    trk=[keys[i] for i in tri]; trl=labels[tri].tolist()
    vak=[keys[i] for i in vai]; val_=labels[vai].tolist()
    log.info("  Train (labeled): %d | Val: %d",len(trk),len(vak))

    pseudo_thresh = 0.85
    if V67_PROBS.exists():
        pdata = torch.load(V67_PROBS, map_location="cpu")
        pprobs = pdata["probs"]
        pkeys = pdata["keys"]
        max_p, ppreds = pprobs.max(1)
        mask = max_p >= pseudo_thresh
        pk = [pkeys[i] for i in range(len(pkeys)) if mask[i]]
        pl = ppreds[mask].tolist()
        trk.extend(pk); trl.extend(pl)
        log.info("  Pseudo-labels (>%.2f): %d added",pseudo_thresh,len(pk))
    log.info("  Total train: %d",len(trk))

    all_cache = {**train_cache, **eval_cache}
    tds=RadarDataset(trk,trl,all_cache,augment=True,img_size=CFG["img_size"],max_frames=CFG["max_frames"])
    ...
    crit = FocalCE(gamma=2.0, smoothing=CFG["label_smoothing"])
