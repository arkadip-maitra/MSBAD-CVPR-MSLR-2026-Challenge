#!/usr/bin/env python3
"""
v41: ConvNeXtV2-Tiny @ 384 with COSINE ANNEALING WARM RESTARTS.
Born-again from v18. Uses CosineAnnealingWarmRestarts (T_0=20, T_mult=2)
instead of OneCycleLR. Multiple LR restarts push the model through different
loss basins → genuinely different solution → different error patterns.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v41")
...
def main():
    ...
    for fold,(tri,vai) in enumerate(skf.split(keys,labels)):
        ...
        if ba.exists():
            ...
            ba2=V11_DIR/f"best_fold{fold}.pt"
        ...
        # Stage 2: full fine-tune + LLRD + COSINE WARM RESTARTS
        log.info("── Stage 2: full fine-tune (%d ep, CosineWarmRestarts T0=%d Tmult=%d) ──",
                 CFG["stage2_epochs"], CFG["cosine_T0"], CFG["cosine_Tmult"])
        ...
        sched=torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
            opt, T_0=CFG["cosine_T0"]*len(tl), T_mult=CFG["cosine_Tmult"])
        pat=0
