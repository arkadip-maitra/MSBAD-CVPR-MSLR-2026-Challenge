#!/usr/bin/env python3
"""
v17: ConvNeXtV2-Tiny @ NATIVE 64×256 resolution + SWA
BREAKTHROUGH IDEA: Stop distorting radar data into 224×224 squares.
The data is 48×256 (time × range). Resizing to 224×224 stretches time by 4.67×
and introduces interpolation artifacts. Instead, pad temporal to 64 frames and
keep native 256 range bins. The model sees REAL radar data without distortion.

ConvNeXtV2 is fully convolutional → handles 64×256 natively.
Born-again from v11 → weights transfer perfectly (conv kernels are size-independent).
SWA → averages weights from multiple training steps for flatter optimum.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v17")
...
CFG = {
    ...
    "max_frames": 64, "img_size": None, "n_classes": 126,
...
def main():
    ...
    log.info("★★★ NATIVE RESOLUTION: %s × 256 (NO RESIZE!) ★★★", CFG["max_frames"])
