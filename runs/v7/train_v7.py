...
V7: ConvNeXt-Tiny + Two-Stage + Knowledge Distillation from v4+v6 ensemble.

Improvements over v6 (86.20%):
  - KNOWLEDGE DISTILLATION: 10-model teacher ensemble (5×v4 + 5×v6) produces
    soft labels for all training samples. Loss = α·CE(pred,y) + (1-α)·τ²·KL(pred/τ, teacher/τ)
    → individual student learns inter-class relationships from ensemble
  - Soft labels mixed consistently with Mixup/CutMix hard labels
  - Temperature τ=3 softens teacher distribution to reveal dark knowledge
  - All v6 improvements: two-stage training, drop_path=0.1, GeM, LLRD

Usage:
    conda run -n slr python /data/slr/train_v7.py
"""
...
OUT_DIR = Path("/data/slr/checkpoints_v7")
...
def precompute_teacher_labels(keys, cache, teacher_dirs, dev):
    ...
    for tdir in teacher_dirs:
        for fold in range(CFG["n_folds"]):
            ...
            if not ckpt_path.exists():
                log.info("  Skipping missing %s", ckpt_path)
                continue
            m = ConvNeXtGeM(
    ...
    log.info("  Loaded %d teacher models from %d dirs", n_teachers, len(teacher_dirs))
    ...
    log.info("  Teacher soft labels ready: %d samples", len(soft_dict))
    return soft_dict


# ===========================================================================
#  MIXUP + CUTMIX (returns permutation for soft label alignment)
# ===========================================================================
...
def kd_loss_fn(logits, soft_target, tau):
    """KL-div between student (logits) and teacher (probs) at temperature τ."""
...
def main():
    ...
    teacher_dirs = []
    for d in [V6_DIR, V4_DIR]:
        if d.exists() and (d / "best_fold0.pt").exists():
            teacher_dirs.append(d)
    if not teacher_dirs:
        log.error("No teacher checkpoints found! Aborting.")
        return
    ...
    alpha = CFG["kd_alpha"]
    tau = CFG["kd_tau"]
    log.info("KD: α=%.2f  τ=%.1f  teachers=%d dirs", alpha, tau, len(teacher_dirs))
    ...
    log.info("Loading v7 checkpoints …")
    v7_models = []
    for fold in range(CFG["n_folds"]):
    ...
    probs_no = predict_ensemble_tta(v7_models, test_loader, dev, use_tta=False)
    ...
    probs_v7 = predict_ensemble_tta(v7_models, test_loader, dev, use_tta=True)
    preds_v7 = probs_v7.argmax(dim=1).numpy()
    _write_submission(test_keys, preds_v7, out / "submission_tta.csv")
    log.info("Submission (TTA) → %s", out / "submission_tta.csv")
    changed = (preds_v7 != preds_no).sum()
    log.info("TTA changed %d / %d (%.1f%%)", changed, len(preds_v7),
             100 * changed / len(preds_v7))
    del v7_models

    # ---- Multi-model ensemble: v4 + v6 + v7 ------------------------------
    all_probs = [probs_v7]
    model_names = ["v7"]
    for vdir, vname in [(V6_DIR, "v6"), (V4_DIR, "v4")]:
        if vdir.exists() and (vdir / "best_fold0.pt").exists():
            log.info("Loading %s checkpoints …", vname)
            models = []
            for fold in range(CFG["n_folds"]):
                m = ConvNeXtGeM(CFG["model_name"], CFG["n_classes"], pretrained=False).to(dev)
                ckpt = torch.load(vdir / f"best_fold{fold}.pt", map_location=dev, weights_only=True)
                m.load_state_dict(ckpt["model_state_dict"])
                models.append(m)
            probs = predict_ensemble_tta(models, test_loader, dev, use_tta=True)
            all_probs.append(probs)
            model_names.append(vname)
            del models

    if len(all_probs) > 1:
        probs_mega = torch.stack(all_probs).mean(dim=0)
        preds_mega = probs_mega.argmax(dim=1).numpy()
        ens_name = "+".join(model_names)
        _write_submission(test_keys, preds_mega, out / f"submission_{ens_name}_ensemble.csv")
        log.info("Submission (%s ensemble) → %s", ens_name,
                 out / f"submission_{ens_name}_ensemble.csv")
        diff = (preds_mega != preds_v7).sum()
        log.info("Mega-ensemble differs from v7-only in %d predictions", diff)
