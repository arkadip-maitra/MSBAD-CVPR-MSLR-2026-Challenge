#!/usr/bin/env python3
"""
v109: 5-fold CV — ConvNeXtV2-Tiny born-again from v106, WEAK augmentation,
CosineAnnealingWarmRestarts (T0=50, Tmult=2) with higher LR 1e-4.
600 epochs, NO early stopping. Test if CAWR can escape OneCycleLR plateau.
Usage: python train_5fold_v109.py --fold 0 --gpu 0
"""
...
CFG = {
    ...
    "stage2_epochs": 600, "stage2_lr": 1e-4,
    "stage2_T_0": 50, "stage2_T_mult": 2,
...
def train_epoch(model, loader, opt, sched, crit, dev, scaler, use_mix=True, step_per_batch=False):
    ...
    for x,y in tqdm(loader,desc="  train",leave=False):
        ...
        if sched is not None and step_per_batch: sched.step()
...
def main():
    ...
    log.info("5-fold CV | %s | Fold %d | WEAK aug | CAWR T0=50 | LR 1e-4 | 600 ep", VERSION, TARGET_FOLD)
    ...
    log.info("── Stage 2: full fine-tune (%d ep, CAWR T0=%d Tmult=%d, NO early stopping) ──",
             CFG["stage2_epochs"], CFG["stage2_T_0"], CFG["stage2_T_mult"])
    ...
    sched=torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(opt,T_0=CFG["stage2_T_0"],T_mult=CFG["stage2_T_mult"],eta_min=1e-7)
    for ep in range(1,CFG["stage2_epochs"]+1):
        t0=time.time()
        trl_,tra_=train_epoch(model,tl,opt,sched,crit,dev,scaler,use_mix=True,step_per_batch=True)
        _,va_=eval_epoch(model,vl,crit,dev)
