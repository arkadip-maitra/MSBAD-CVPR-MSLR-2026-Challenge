#!/usr/bin/env python3
"""
v62: EVA02-Base @ 224 (86M params) — single fold, 8-teacher KD, heavy regularization.
MIM-pretrained ViT with uniform 768-dim features. Custom multi-scale head for ViT.
"""
import os
...
import csv, json, logging, time, re
...
OUT_DIR = Path("/data/slr/checkpoints_v62")
...
CFG = {
    ...
    "n_folds": 5, "batch_size": 32, "num_workers": 2,
    "stage1_epochs": 5, "stage1_lr": 1e-4,
    "stage2_epochs": 200, "stage2_lr": 3e-5,
    "swa_epochs": 30, "swa_lr": 3e-6,
    "weight_decay": 0.08, "warmup_pct": 0.08,
    "label_smoothing": 0.15, "early_stopping_patience": 50,
    "grad_clip_norm": 1.0, "llrd_decay": 0.90,
    ...
    "random_erase_prob": 0.25, "circ_shift_prob": 0.3, "circ_shift_max": 8,
    "amp_scale_prob": 0.5, "amp_scale_lo": 0.8, "amp_scale_hi": 1.2,
    "model_name": "eva02_base_patch14_224.mim_in22k",
    "drop_path_rate": 0.3, "proj_dim": 768, "drop_rate": 0.4,
...
class FullEVA02(nn.Module):
    """EVA02 with full backbone (num_classes=0 → CLS/pooled features) + MultiSampleDropout head."""
    def __init__(self, model_name, n_classes=126, pretrained=True,
                 drop_path_rate=0.3, proj_dim=768, drop_rate=0.4, ms_samples=5,
                 **kwargs):
        super().__init__()
        self.backbone = timm.create_model(model_name, pretrained=pretrained, num_classes=0,
                                          drop_path_rate=drop_path_rate)
        embed_dim = self.backbone.embed_dim
        self.proj = nn.Sequential(nn.Linear(embed_dim, proj_dim), nn.LayerNorm(proj_dim), nn.GELU())
...
def get_llrd_params(model, base_lr, decay=0.90, wd=0.08):
    """Layer-wise LR decay for EVA02 ViT: deeper blocks get higher LR."""
    groups = {}
    n_blocks = 12
...
def main():
    ...
    log.info("Student: EVA02-Base@224 (86M) — SINGLE FOLD %d", CFG["run_fold"])
    ...
    log.info("KD α=%.2f τ=%.1f | SWA %d ep | SINGLE FOLD | 8-teacher KD | EVA02 MIM",alpha,tau,CFG["swa_epochs"])
    ...
    for fi,(tri,vai) in enumerate(skf.split(keys,labels)):
        ...
        model=FullEVA02(CFG["model_name"],CFG["n_classes"],pretrained=True,
        ...
        log.info("  Updating BN/LN stats for SWA model …")
    ...
    m=FullEVA02(CFG["model_name"],CFG["n_classes"],pretrained=False,drop_path_rate=0.0,
