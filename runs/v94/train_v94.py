#!/usr/bin/env python3
"""
v94: SAM (Sharpness-Aware Minimization) optimizer — seeks flat minima.
Born-again from v67. SAM adds a perturbation step before the gradient step,
encouraging the optimizer to find solutions that generalize better.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v94")
...
class SAM:
    """Sharpness-Aware Minimization wrapper (composition, not inheritance)."""
    ...
    def second_step(self):
        for group in self.param_groups:
            for p in group["params"]:
                if p.grad is None: continue
                if id(p) in self._e_w:
                    p.sub_(self._e_w[id(p)])
...
def train_epoch_sam(model, loader, opt, sched, crit, dev, scaler, use_mix=True):
    """SAM training: two forward passes per batch. No AMP scaler with SAM for simplicity."""
    ...
    for x,y in tqdm(loader,desc="  train",leave=False):
        ...
        with torch.amp.autocast('cuda'):
            logits2=model(xm)
            loss2=lam*crit(logits2,ya)+(1-lam)*crit(logits2,yb)
        opt.zero_grad(set_to_none=True)
...
def main():
    ...
    log.info("── Stage 2: SAM fine-tune (%d ep, rho=%.3f, patience %d) ──",CFG["stage2_epochs"],CFG["sam_rho"],CFG["early_stopping_patience"])
    for p in model.parameters(): p.requires_grad=True
    pg=get_llrd_params(model,CFG["stage2_lr"],decay=CFG["llrd_decay"],wd=CFG["weight_decay"])
    for g in pg: g["rho"] = CFG["sam_rho"]
    opt=SAM(pg, AdamW, rho=CFG["sam_rho"], lr=CFG["stage2_lr"], weight_decay=CFG["weight_decay"])
    sched=torch.optim.lr_scheduler.OneCycleLR(opt.base_optimizer,max_lr=[g["lr"] for g in pg],
    ...
    for ep in range(1,CFG["stage2_epochs"]+1):
        t0=time.time()
        trl_,tra_=train_epoch_sam(model,tl,opt,sched,crit,dev,scaler,use_mix=True)
        _,va_=eval_epoch(model,vl,crit,dev)
