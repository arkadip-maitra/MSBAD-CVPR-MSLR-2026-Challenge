#!/usr/bin/env python3
"""
v35: ConvNeXtV2-Tiny @ 224 with CONFUSION-AWARE LABEL SMOOTHING + SWA + KD.
Instead of uniform label smoothing (each wrong class gets equal probability),
we use the OOF confusion matrix to create class-specific soft labels:
- Classes that our models frequently confuse get MORE smoothed probability
- This forces the model to focus on distinguishing hard, confusable pairs
- Genuinely different training signal from all previous models

Born-again from v11. KD from v11+vit_v9.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v35")
...
CFG = {
    ...
    "n_folds": 5, "batch_size": 48, "num_workers": 4,
    "stage1_epochs": 5, "stage1_lr": 2e-4,
    "stage2_epochs": 100, "stage2_lr": 5e-5,
    "swa_epochs": 15, "swa_lr": 5e-6,
    "weight_decay": 0.05, "warmup_pct": 0.05,
    "label_smoothing": 0.1,
    "confusion_smoothing_weight": 0.5,
...
class RadarDataset(Dataset):
    ...
    def __getitem__(self, idx):
        x=self.cache[self.keys[idx]].copy(); x=(x-_MEANS)/_STDS
...
def build_confusion_labels(keys, labels, cache, teacher_configs, dev, n_classes=126, seed=42, n_folds=5):
    """
    Build confusion-aware soft labels using OOF predictions from teachers.
    For each class c, compute P(predicted=j | true=c) from the confusion matrix.
    The soft label for class c becomes a weighted mix of one-hot and confusion row.
    """
    log.info("Building confusion-aware soft labels from OOF predictions...")
    n = len(keys)
    oof_probs = torch.zeros(n, n_classes)

    ds = RadarDataset(keys, [0]*n, cache, augment=False, img_size=224, max_frames=48)
    teachers = []
    for cfg in teacher_configs:
        td = cfg["dir"]
        if not td.exists(): continue
        kw = {"n_classes": n_classes, "pretrained": False, **cfg["kwargs"]}
        for f in range(n_folds):
            ...
            m.eval()
            teachers.append((m, f))
    log.info("  Loaded %d teacher checkpoints", len(teachers))

    skf = StratifiedKFold(n_splits=n_folds, shuffle=True, random_state=seed)
    fold_indices = {}
    for fold, (tri, vai) in enumerate(skf.split(keys, labels)):
        fold_indices[fold] = vai

    for fold, vai in fold_indices.items():
        fold_teachers = [m for m, f in teachers if f == fold]
        if not fold_teachers: continue
        val_keys = [keys[i] for i in vai]
        val_ds = RadarDataset(val_keys, [0]*len(val_keys), cache, augment=False, img_size=224, max_frames=48)
        loader = DataLoader(val_ds, batch_size=96, shuffle=False, num_workers=4, pin_memory=True)
        all_probs = []
        with torch.no_grad():
            for x, _, _ in loader:
                x = x.to(dev, non_blocking=True)
                avg = sum(F.softmax(m(x), 1) for m in fold_teachers) / len(fold_teachers)
                all_probs.append(avg.cpu())
        oof_probs[vai] = torch.cat(all_probs)
    ...
    for true, pred_probs in zip(labels, oof_probs.numpy()):
        conf_matrix[true] += pred_probs

    for c in range(n_classes):
        row_sum = conf_matrix[c].sum()
        if row_sum > 0:
            conf_matrix[c] /= row_sum

    confusion_labels = torch.from_numpy(conf_matrix).float()
    log.info("  Confusion matrix built. Example row 0: top-5 confused = %s",
             torch.topk(confusion_labels[0], 5))

    oof_acc = (oof_preds == labels).mean()
    log.info("  OOF teacher accuracy: %.2f%%", 100 * oof_acc)

    return confusion_labels

class ConfusionAwareCE(nn.Module):
    """
    Cross-entropy with confusion-aware label smoothing.
    Instead of uniform smoothing, distribute the smoothing mass according to
    how often classes are confused in practice.
    """
    def __init__(self, confusion_labels, smoothing=0.1, conf_weight=0.5):
        super().__init__()
        self.register_buffer("confusion_labels", confusion_labels)
        self.smoothing = smoothing
        self.conf_weight = conf_weight
        self.n_classes = confusion_labels.size(0)

    def forward(self, logits, targets):
        with torch.no_grad():
            one_hot = F.one_hot(targets, self.n_classes).float()
            conf_row = self.confusion_labels[targets]
            conf_row_no_diag = conf_row.clone()
            conf_row_no_diag.scatter_(1, targets.unsqueeze(1), 0.0)
            row_sum = conf_row_no_diag.sum(1, keepdim=True).clamp(min=1e-8)
            conf_row_no_diag = conf_row_no_diag / row_sum

            uniform = torch.ones_like(one_hot) / self.n_classes
            smoothing_dist = self.conf_weight * conf_row_no_diag + (1 - self.conf_weight) * (1.0 / (self.n_classes - 1)) * (1 - one_hot)
            smoothing_dist = smoothing_dist / smoothing_dist.sum(1, keepdim=True)

            soft_target = (1 - self.smoothing) * one_hot + self.smoothing * smoothing_dist
...
def get_llrd_params(model, base_lr, decay=0.75, wd=0.05):
    ...
    for name,p in model.named_parameters():
        ...
        if "backbone.stem" in name or "backbone.stages_0" in name: lr=base_lr*decay**4
...
def precompute_teacher_labels(keys, cache, configs, dev):
    ...
    ds=RadarDataset(keys,[0]*len(keys),cache,augment=False,img_size=224,max_frames=48)
...
def main():
    ...
    log.info("★★★ CONFUSION-AWARE LABEL SMOOTHING ★★★")
    ...
    confusion_labels = build_confusion_labels(
        keys, labels, train_cache, TEACHER_CONFIGS, dev,
        n_classes=CFG["n_classes"], seed=CFG["seed"], n_folds=CFG["n_folds"]
    )
    ...
    for fold,(tri,vai) in enumerate(skf.split(keys,labels)):
        ...
        ba=V11_DIR/f"best_fold{fold}.pt"
        if ba.exists():
            ck=torch.load(ba,map_location=dev,weights_only=True)
            model.load_state_dict(ck["model_state_dict"])
            log.info("  Born-again from v11 fold %d (val %.2f%%)",fold,100*ck.get("val_acc",0))

        crit=ConfusionAwareCE(confusion_labels.to(dev),
                              smoothing=CFG["label_smoothing"],
                              conf_weight=CFG["confusion_smoothing_weight"])
        ...
        for ep in range(1,CFG["stage1_epochs"]+1):
            ...
            _,va_=eval_epoch(model,vl,eval_crit,dev)
        ...
        log.info("── Stage 2: full fine-tune (%d ep) ──",CFG["stage2_epochs"])
        ...
        log.info("── Stage 3: SWA (%d ep) ──",CFG["swa_epochs"])
    ...
    eds=RadarDataset(eval_keys,[0]*len(eval_keys),eval_cache,augment=False,img_size=CFG["img_size"],max_frames=CFG["max_frames"])
