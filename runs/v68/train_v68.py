#!/usr/bin/env python3
"""
v68: Born-again chain v11 → v66 → v68. Second generation self-distillation.
v66 achieved 90.84% on fold 0 (better than v11's 90.78%).
Train a new model born-again from v66 to iteratively improve.
ALSO uses CLIP-based semantic label smoothing: similar signs get more smoothing weight.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v68")
...
def compute_smooth_matrix(idx_to_cls, dev, tau=0.05):
    """Compute CLIP-based semantic smoothing matrix."""
    ...
    smooth = F.softmax(sim / tau, dim=-1)
    mask = ~torch.eye(126, device=dev).bool()
    log.info("Semantic smooth matrix: mean_off_diag=%.4f, max_off_diag=%.4f",
             smooth[mask].mean().item(), smooth[mask].max().item())
...
class SemanticCE(nn.Module):
    """Cross-entropy with CLIP-based semantic label smoothing."""
    def __init__(self, smooth_matrix, smoothing=0.15):
        super().__init__()
        self.register_buffer('smooth_matrix', smooth_matrix)
        self.smoothing = smoothing
    def forward(self, logits, targets):
        hard = F.one_hot(targets, logits.size(-1)).float()
        soft = self.smooth_matrix[targets]
        labels = (1 - self.smoothing) * hard + self.smoothing * soft
...
def eval_epoch(model, loader, dev):
    ...
    for x,y in tqdm(loader,desc="  val  ",leave=False):
        x,y=x.to(dev,non_blocking=True),y.to(dev,non_blocking=True)
        with torch.amp.autocast('cuda'):
            lo=model(x); co+=lo.argmax(1).eq(y).sum().item(); n+=x.size(0)
    return co/n
...
def main():
    ...
    log.info("Born-again chain: v11→v66→v68 | Semantic smoothing=%.2f | Fold %d", SEMANTIC_SMOOTHING, TARGET_FOLD)
    ...
    smooth_matrix = compute_smooth_matrix(idx_to_cls, dev)

    skf=StratifiedKFold(n_splits=CFG["n_folds"],shuffle=True,random_state=CFG["seed"])
    for fold,(tri,vai) in enumerate(skf.split(keys,labels)):
        if fold == TARGET_FOLD: break
    log.info("="*64); log.info("FOLD %d — BORN-AGAIN CHAIN + SEMANTIC SMOOTHING",TARGET_FOLD); log.info("="*64)
    ...
    if ba.exists():
        ...
        log.warning("v66 checkpoint not found, falling back to v11")
        from pathlib import Path as P
        ba2 = P("/data/slr/checkpoints_v11")/f"best_fold{TARGET_FOLD}.pt"
        if ba2.exists(): load_born_again(model,ba2,dev)

    scaler = torch.amp.GradScaler('cuda')
    crit = SemanticCE(smooth_matrix, smoothing=SEMANTIC_SMOOTHING)
    ...
    for ep in range(1,CFG["stage1_epochs"]+1):
        t0=time.time()
        trl_,tra_=train_epoch(model,tl,opt,sched,crit,dev,scaler,use_mix=False)
        va_=eval_epoch(model,vl,dev)
    ...
    for ep in range(1,CFG["stage2_epochs"]+1):
        t0=time.time()
        trl_,tra_=train_epoch(model,tl,opt,sched,crit,dev,scaler,use_mix=True)
        va_=eval_epoch(model,vl,dev)
    ...
    for swa_ep in range(1,CFG["swa_epochs"]+1):
        ...
        sva=eval_epoch(swa_model,vl,dev)
    ...
    swa_acc=eval_epoch(swa_model,vl,dev)
