#!/usr/bin/env python3
"""
v86: ConvNeXtV2-Tiny @ 384x384 resolution — higher spatial detail for
distinguishing confusing alphabet gestures. Born-again from v66 (224x224).
ConvNeXt is resolution-agnostic (fully convolutional), so weights transfer directly.
Batch size reduced to 20 for memory.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v86")
...
CFG = {
    ...
    "stage2_epochs": 150, "stage2_lr": 3e-5,
    "swa_epochs": 20, "swa_lr": 3e-6,
    ...
    "time_mask_max": 12, "range_mask_max": 45, "noise_std": 0.1,
...
class RadarDataset(Dataset):
    def __init__(self, keys, labels, cache, augment=False, img_size=384, max_frames=48):
...
def _tta(x):
    return [x,x.flip(2),x.roll(-6,2),x.roll(6,2),x[:,[1,2,0],:,:],x[:,[2,0,1],:,:],
...
def main():
    ...
    log.info("ConvNeXtV2-Tiny @ 384x384 | born-again v66 | Fold %d", TARGET_FOLD)
    ...
    log.info("="*64); log.info("FOLD %d — ConvNeXtV2-Tiny @ 384x384",TARGET_FOLD); log.info("="*64)
