...
ViT v7: Swin-Small + Pseudo-labeling + Two-stage born-again from v6.

Improvements over ViT v6 (84.68% CV):
  - PSEUDO-LABELING: v7+v6 CNN teacher ensemble labels 9828 eval samples.
    High-confidence pseudo-labels (≥95%) become extra training data,
    adding ~50-70% more samples and reducing the train/eval distribution gap.
  - TWO-STAGE TRAINING REINSTATED: v6's single-stage limited head adaptation.
    Stage 1: head-only warmup (10 epochs, frozen backbone).
    Stage 2: full fine-tuning with LLRD + Mixup/CutMix.
  - BORN-AGAIN FROM v6: initialise from v6 checkpoints (~84.68%)
    rather than v5 (~84.63%), starting from a better basin.
  - ADJUSTED SCHEDULE: lower S2 LR (3e-5), longer patience (40).

Usage:
    conda run -n slr python /data/slr/train_vit_v7.py
"""
...
OUT_DIR = Path("/data/slr/checkpoints_vit_v7")
VIT6_DIR = Path("/data/slr/checkpoints_vit_v6")
...
# ===========================================================================
#  BORN-AGAIN: load v6 checkpoint into SwinAvgPool
# ===========================================================================


def load_born_again(model, ckpt_path, dev):
    """Load v6 SwinAvgPool checkpoint (same architecture)."""
    ckpt = torch.load(ckpt_path, map_location=dev, weights_only=True)
    v6_state = ckpt["model_state_dict"]

    new_state = model.state_dict()
    loaded, skipped = 0, 0
    for k, v in v6_state.items():
...
# ===========================================================================
#  PSEUDO-LABELING from CNN teachers
# ===========================================================================
...
def generate_pseudo_labels(eval_keys, eval_cache, teacher_dirs, dev,
    ...
    n_t = len(teachers)
    log.info("  Loaded %d CNN teacher models for pseudo-labeling", n_t)
...
# ===========================================================================
#  TEACHER SOFT LABELS (KD)
# ===========================================================================


def precompute_teacher_labels(keys, cache, dev):
    log.info("Pre-computing teacher soft labels (v7+v6 CNN) …")
...
def main():
    ...
    pl_keys, pl_labels, pl_soft = generate_pseudo_labels(
        eval_keys, eval_cache, [V7_DIR, V6_DIR], dev,
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(keys, labels)):
        ...
        # ---- Build model + born-again init from v6 -----------------------
        model = SwinAvgPool(
            CFG["model_name"], CFG["n_classes"], CFG["pretrained"],
            drop_path_rate=CFG["drop_path_rate"],
        ).to(dev)

        v6_ckpt = VIT6_DIR / f"best_fold{fold}.pt"
        if v6_ckpt.exists():
            load_born_again(model, v6_ckpt, dev)
        else:
            log.info("  No v6 checkpoint for fold %d, using ImageNet init", fold)
        ...
        # ==== STAGE 1: Head-only warmup + KD ================================
        log.info("── Stage 1: head-only + KD (%d epochs) ──",
                 CFG["stage1_epochs"])
        ...
        param_groups = get_swin_param_groups(
            model, CFG["stage2_lr"], CFG["llrd_stage_decay"],
            CFG["weight_decay"],
        )
    ...
    log.info("Loading ViT v7 checkpoints …")
    vit_models = []
    for fold in range(CFG["n_folds"]):
        m = SwinAvgPool(CFG["model_name"], CFG["n_classes"],
    ...
    probs_no = predict_ensemble_tta(vit_models, eval_loader, dev, use_tta=False)
    ...
    probs_vit = predict_ensemble_tta(vit_models, eval_loader, dev, use_tta=True)
    preds_vit = probs_vit.argmax(dim=1).numpy()
    _write_submission(eval_keys, preds_vit, out / "submission_tta.csv")
    ...
    # Save probs for final ensemble
    torch.save({"keys": eval_keys, "probs": probs_vit},
               out / "eval_probs_tta.pt")
    log.info("Saved vit_v7 TTA probs → %s", out / "eval_probs_tta.pt")

    # ---- Cross-arch ensemble: ViT v7 + CNN v7 ----------------------------
    if V7_DIR.exists() and (V7_DIR / "best_fold0.pt").exists():
        ...
        for fold in range(CFG["n_folds"]):
            ...
            m.load_state_dict(ckpt["model_state_dict"])
            cnn_models.append(m)
        probs_cnn = predict_ensemble_tta(cnn_models, eval_loader, dev,
                                         use_tta=True)
        del cnn_models
        torch.cuda.empty_cache()
        ...
        _write_submission(eval_keys, preds_ens,
                          out / "submission_vit7+cnn7_ensemble.csv")
        log.info("Submission (ViT v7 + CNN v7) → %s",
                 out / "submission_vit7+cnn7_ensemble.csv")
