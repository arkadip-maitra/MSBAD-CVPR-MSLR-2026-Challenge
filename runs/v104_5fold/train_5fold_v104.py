#!/usr/bin/env python3
"""
v104: 5-fold CV — ConvNeXtV2-BASE (3x capacity). From ImageNet pretrain only.
Larger batch, higher regularization, warm restarts.
Usage: python train_5fold_v104.py --fold 0 --gpu 0
"""
...
def main():
    ...
    log.info("5-fold CV | %s | Fold %d | ConvNeXtV2-BASE | CosineWarmRestarts", VERSION, TARGET_FOLD)
    ...
    log.info("="*64); log.info("FOLD %d — ConvNeXtV2-BASE from ImageNet",TARGET_FOLD); log.info("="*64)
    ...
    log.info("── Stage 2: CosineWarmRestarts (%d ep, T0=%d, Tmult=%d, lr=%.1e, patience %d) ──",
             CFG["stage2_epochs"], CFG["stage2_T_0"], CFG["stage2_T_mult"], CFG["stage2_lr"], CFG["early_stopping_patience"])
    ...
    T_0_steps = CFG["stage2_T_0"] * steps_per_epoch
    sched = CosineAnnealingWarmRestarts(opt, T_0=T_0_steps, T_mult=CFG["stage2_T_mult"], eta_min=1e-7)
