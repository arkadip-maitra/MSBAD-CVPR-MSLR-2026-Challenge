#!/usr/bin/env python3
"""
v49: ConvNeXtV2-Base @ 224×224 + SWA. Larger backbone for higher capacity.
ImageNet-pretrained student; KD from v11 (ConvNeXtV2-Tiny) + vit_v9 (CAFormer).
No born-again (Base vs Tiny channel dimensions differ).
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v49")
...
def main():
    ...
    log.info("★★★ v49 ConvNeXtV2-Base @ %s×%s (ImageNet init, no born-again) ★★★", CFG["img_size"], CFG["img_size"])
    ...
    log.info("KD α=%.2f τ=%.1f | SWA %d ep @ lr=%.1e | NO PL | teachers: v11 + vit_v9",alpha,tau,CFG["swa_epochs"],CFG["swa_lr"])
