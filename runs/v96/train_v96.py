#!/usr/bin/env python3
"""
v96: Train from scratch (no ImageNet pretrain, no born-again).
Tests whether ImageNet features help or hinder for radar data.
Extended training (300ep) with higher LR and longer warmup.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v96")
...
CFG = {
    ...
    "stage1_epochs": 10, "stage1_lr": 5e-4,
    "stage2_epochs": 300, "stage2_lr": 1e-4,
    "swa_epochs": 30, "swa_lr": 1e-5,
    "weight_decay": 0.05, "warmup_pct": 0.10,
    "label_smoothing": 0.1, "early_stopping_patience": 50,
    "grad_clip_norm": 1.0, "llrd_decay": 1.0,
...
def main():
    ...
    model=MultiScaleConvNeXtV2(CFG["model_name"],CFG["n_classes"],pretrained=False,
                               drop_path_rate=CFG["drop_path_rate"],proj_dim=CFG["proj_dim"],
                               drop_rate=CFG["drop_rate"],ms_samples=CFG["ms_dropout_samples"]).to(dev)
    log.info("  Training from SCRATCH (no pretrain, no born-again)")
