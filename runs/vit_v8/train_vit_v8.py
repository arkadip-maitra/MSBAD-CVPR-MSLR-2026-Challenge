...
ViT v8: CAFormer-S18 (MetaFormer) + Multi-Scale Feature Aggregation

Key innovations over vit_v7 (Swin-Small, 85.17%):
    1. CAFormer-S18 backbone — fundamentally different from Swin:
       - Stages 1–2: pooling-based token mixing (captures local patterns efficiently)
       - Stages 3–4: standard self-attention (captures global dependencies)
       - MetaFormer principle: architecture > attention mechanism
       - 23.2M params (compact, regularization-friendly)
       - IN22k pretraining
    2. Multi-scale feature aggregation from all 4 stages
       [64, 128, 320, 512] → project to 384-dim → attention-weighted combination
    3. Multi-teacher KD from v7+v6 CNN teachers
    4. Pseudo-labels at threshold 0.70

Usage:
    conda run -n slr python /data/slr/train_vit_v8.py
"""
...
OUT_DIR = Path("/data/slr/checkpoints_vit_v8")
...
# ===========================================================================
#  MODEL — CAFormer + Multi-Scale Feature Aggregation
# ===========================================================================
...
class MultiScaleCAFormer(nn.Module):
    """CAFormer with multi-scale feature aggregation.

    CAFormer (MetaFormer) key property:
      - Stages 1-2: Pooling token mixer (local, efficient, like convolutions)
      - Stages 3-4: Attention token mixer (global, captures long-range deps)
    This hybrid is fundamentally different from Swin's shifted-window approach.
    """
...
# ===========================================================================
#  TEACHER (ConvNeXt v1 for v7/v6 checkpoints)
# ===========================================================================
...
def main():
    ...
    log.info("Loading vit_v8 checkpoints for ensemble …")
    models_v8 = []
    for fold in range(CFG["n_folds"]):
    ...
    probs_no = predict_ensemble_tta(models_v8, eval_loader, dev, use_tta=False)
    ...
    probs_v8 = predict_ensemble_tta(models_v8, eval_loader, dev, use_tta=True)
    preds_v8 = probs_v8.argmax(dim=1).numpy()
    _write_submission(eval_keys, preds_v8, out / "submission_tta.csv")
    ...
    torch.save({"keys": eval_keys, "probs": probs_v8},
               out / "eval_probs_tta.pt")
    log.info("Saved vit_v8 TTA probs → %s", out / "eval_probs_tta.pt")
