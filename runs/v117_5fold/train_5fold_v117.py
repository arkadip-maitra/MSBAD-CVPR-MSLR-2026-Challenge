#!/usr/bin/env python3
"""
v117: 5-fold CV — ConvNeXtV2-Tiny born-again from v113, WEAK augmentation,
FOCAL LOSS (gamma=2). 384x384 resolution (up from 288). 400 epochs.
Tests if even higher resolution helps with fine-grained hand shape classification.
Usage: python train_5fold_v117.py --fold 0 --gpu 0
"""
...
VERSION = "v117"
PARENT_DIR = Path("/data/slr/checkpoints_v113")
...
CFG = {
    ...
    "max_frames": 48, "img_size": 384, "n_classes": 126,
    "focal_gamma": 2.0,
    ...
    "stage2_epochs": 400, "stage2_lr": 5e-5,
...
class FocalLoss(nn.Module):
    def __init__(self, gamma=2.0, label_smoothing=0.0, n_classes=126):
        super().__init__()
        self.gamma = gamma
    ...
    def forward(self, logits, targets):
        if self.label_smoothing > 0:
            ...
            targets_smooth = one_hot * (1 - self.label_smoothing) + smooth
...
def main():
    ...
    log.info("5-fold CV | %s | Fold %d | WEAK aug | Focal(γ=%.1f) | 288px | 400 ep", VERSION, TARGET_FOLD, CFG["focal_gamma"])
    ...
    crit=FocalLoss(gamma=CFG["focal_gamma"],label_smoothing=CFG["label_smoothing"],n_classes=CFG["n_classes"])
    ...
    log.info("── Stage 2: full fine-tune (%d ep, NO early stopping) ──", CFG["stage2_epochs"])
