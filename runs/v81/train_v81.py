#!/usr/bin/env python3
"""
v81: SwinV2-Tiny backbone — attention-based architecture for diversity.
Same stage dims [96,192,384,768] as ConvNeXtV2-Tiny, so head weights transfer from v66.
Partial born-again: head weights from v66, backbone from ImageNet pretrained.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v81")
...
CFG = {
    ...
    "stage1_epochs": 10, "stage1_lr": 2e-4,
...
def load_born_again_head_only(model, cp, dev):
    """Load only the head weights (stage_projs, scale_attn, head) from a ConvNeXtV2 checkpoint.
    Works because SwinV2-Tiny and ConvNeXtV2-Tiny share the same stage dims [96,192,384,768]."""
    ck=torch.load(cp,map_location=dev,weights_only=True)
    src_sd = ck["model_state_dict"]
    tgt_sd = model.state_dict()
    loaded = 0
    for name in tgt_sd:
        if name.startswith(("stage_gems","stage_projs","scale_attn","head")):
            if name in src_sd and src_sd[name].shape == tgt_sd[name].shape:
                tgt_sd[name] = src_sd[name]; loaded += 1
    model.load_state_dict(tgt_sd)
    log.info("  Partial born-again: loaded %d/%d head params from %s (val %.2f%%)",
             loaded, len([n for n in tgt_sd if n.startswith(("stage_gems","stage_projs","scale_attn","head"))]),
             cp.name, 100*ck.get("val_acc",0))

def get_llrd_params(model, base_lr, decay=0.75, wd=0.05):
    groups={}
    for name,p in model.named_parameters():
        if not p.requires_grad: continue
        if "backbone.patch_embed" in name:      lr=base_lr*decay**5
        elif "backbone.layers_0" in name:       lr=base_lr*decay**4
        elif "backbone.layers_1" in name:       lr=base_lr*decay**3
        elif "backbone.layers_2" in name:       lr=base_lr*decay**2
        elif "backbone.layers_3" in name:       lr=base_lr*decay**1
        else: lr=base_lr
...
def main():
    ...
    log.info("SwinV2-Tiny backbone | partial born-again head from v66 | Fold %d", TARGET_FOLD)
    ...
    log.info("="*64); log.info("FOLD %d — SwinV2-Tiny backbone",TARGET_FOLD); log.info("="*64)
    ...
    if ba.exists(): load_born_again_head_only(model,ba,dev)
