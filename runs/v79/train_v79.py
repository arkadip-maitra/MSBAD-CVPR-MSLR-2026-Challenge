#!/usr/bin/env python3
"""
v79: EfficientNetV2-S backbone — architecture diversity for ensembling.
Different inductive biases than ConvNeXtV2, smaller stage dims [24,48,64,160].
ImageNet-21k fine-tuned pretrained, no born-again (different arch).
Extended training schedule since we start from ImageNet.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v79")
...
class MultiScaleEfficientNetV2(nn.Module):
    def __init__(self, model_name, n_classes=126, pretrained=True,
        ...
        log.info("EfficientNetV2-S stage dims: %s (total features: %d)", stage_dims, sum(stage_dims))
...
def main():
    ...
    log.info("EfficientNetV2-S backbone — architecture diversity, no born-again | Fold %d", TARGET_FOLD)
    ...
    log.info("="*64); log.info("FOLD %d — EfficientNetV2-S backbone",TARGET_FOLD); log.info("="*64)
    ...
    model=MultiScaleEfficientNetV2(CFG["model_name"],CFG["n_classes"],pretrained=True,
    ...
    log.info("Model params: %.1fM", total_params)
    ...
    m=MultiScaleEfficientNetV2(CFG["model_name"],CFG["n_classes"],pretrained=False,drop_path_rate=0.0,
