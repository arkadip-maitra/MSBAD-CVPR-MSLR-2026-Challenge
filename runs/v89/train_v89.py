#!/usr/bin/env python3
"""
v89: Multi-teacher knowledge distillation — ensemble of v66+v67+v87+v88 (~90.5-91%)
teaches a new student ConvNeXtV2-Tiny. The multi-teacher ensemble captures shared
knowledge while filtering individual noise. Born-again from v66 for the student.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v89")
V66_DIR = Path("/data/slr/checkpoints_v66")
TEACHER_DIRS = [
    Path("/data/slr/checkpoints_v66"),
    Path("/data/slr/checkpoints_v67"),
    Path("/data/slr/checkpoints_v87"),
    Path("/data/slr/checkpoints_v88"),
]
...
def kd_loss(student_logits, teacher_logits, labels, temp, alpha, smoothing=0.1):
    """Combined KD + CE loss. Teacher logits are the averaged logits from the ensemble."""
    soft_student = F.log_softmax(student_logits / temp, dim=-1)
    soft_teacher = F.softmax(teacher_logits / temp, dim=-1)
    kd = F.kl_div(soft_student, soft_teacher, reduction='batchmean') * (temp ** 2)
    ce = F.cross_entropy(student_logits, labels, label_smoothing=smoothing)
    return alpha * kd + (1 - alpha) * ce

def train_epoch_kd(student, teachers, loader, opt, sched, dev, scaler, temp, alpha, use_mix=True):
    student.train()
    ...
    for x, y in tqdm(loader, desc="  train", leave=False):
        ...
        with torch.amp.autocast('cuda'):
            s_logits = student(xm)
            with torch.no_grad():
                t_logits = torch.stack([t(xm) for t in teachers]).mean(0)
            loss = lam * kd_loss(s_logits, t_logits, ya, temp, alpha) + \
                   (1-lam) * kd_loss(s_logits, t_logits, yb, temp, alpha)
        opt.zero_grad(set_to_none=True); scaler.scale(loss).backward()
        scaler.unscale_(opt); nn.utils.clip_grad_norm_(student.parameters(), CFG["grad_clip_norm"])
        scaler.step(opt); scaler.update()
        if sched is not None: sched.step()
        bs = x.size(0); tl += loss.item()*bs
        co += (lam*s_logits.argmax(1).eq(ya).float() + (1-lam)*s_logits.argmax(1).eq(yb).float()).sum().item()
        n += bs
    return tl/n, co/n
...
def main():
    ...
    log.info("Multi-teacher KD (v66+v67+v87+v88) | temp=%.1f alpha=%.2f | Fold %d", KD_TEMP, KD_ALPHA, TARGET_FOLD)
    ...
    for td in TEACHER_DIRS:
        cp = td / f"best_fold{TARGET_FOLD}.pt"
        if cp.exists():
            t = MultiScaleConvNeXtV2(CFG["model_name"], CFG["n_classes"], pretrained=False,
                                     drop_path_rate=0.0, proj_dim=CFG["proj_dim"],
                                     drop_rate=0.0, ms_samples=1).to(dev)
            ck = torch.load(cp, map_location=dev, weights_only=True)
            # Filter out unexpected keys (e.g. CLIP params from v67)
            model_keys = set(t.state_dict().keys())
            filtered_sd = {k: v for k, v in ck["model_state_dict"].items() if k in model_keys}
            t.load_state_dict(filtered_sd, strict=False)
            t.eval()
            for p in t.parameters(): p.requires_grad = False
            teachers.append(t)
            skipped = len(ck["model_state_dict"]) - len(filtered_sd)
            log.info("  Teacher loaded: %s (val %.2f%%, skipped %d extra keys)",
                     td.name, 100*ck.get("val_acc",0), skipped)
    log.info("  Total teachers: %d", len(teachers))

    skf=StratifiedKFold(n_splits=CFG["n_folds"],shuffle=True,random_state=CFG["seed"])
    for fold,(tri,vai) in enumerate(skf.split(keys,labels)):
        if fold == TARGET_FOLD: break
    log.info("="*64); log.info("FOLD %d — Multi-teacher KD",TARGET_FOLD); log.info("="*64)
    ...
    student=MultiScaleConvNeXtV2(CFG["model_name"],CFG["n_classes"],pretrained=True,
    ...
    if ba.exists(): load_born_again(student,ba,dev)

    scaler = torch.amp.GradScaler('cuda')
    crit=nn.CrossEntropyLoss(label_smoothing=CFG["label_smoothing"])
    best_acc=0.0; ckpt=out/f"best_fold{TARGET_FOLD}.pt"

    log.info("── Stage 1: head warmup (%d ep, no KD) ──",CFG["stage1_epochs"])
    for p in student.backbone.parameters(): p.requires_grad=False
    s1p=[p for p in student.parameters() if p.requires_grad]
    ...
    for ep in range(1,CFG["stage1_epochs"]+1):
        t0=time.time()
        trl_,tra_=train_epoch(student,tl,opt,sched,crit,dev,scaler,use_mix=False)
        _,va_=eval_epoch(student,vl,crit,dev)
        log.info("  S1 Ep %d/%d  loss=%.4f acc=%.1f%%  val=%.1f%%  %.0fs",ep,CFG["stage1_epochs"],trl_,100*tra_,100*va_,time.time()-t0)
        if va_>best_acc:
            best_acc=va_; torch.save({"fold":TARGET_FOLD,"epoch":ep,"stage":1,"model_state_dict":student.state_dict(),"val_acc":va_},ckpt)
            log.info("  ★ best %.2f%%",100*va_)
    del opt,sched

    log.info("── Stage 2: full fine-tune WITH multi-teacher KD (%d ep) ──",CFG["stage2_epochs"])
    for p in student.parameters(): p.requires_grad=True
    pg=get_llrd_params(student,CFG["stage2_lr"],decay=CFG["llrd_decay"],wd=CFG["weight_decay"])
    ...
    for ep in range(1,CFG["stage2_epochs"]+1):
        t0=time.time()
        trl_,tra_=train_epoch_kd(student,teachers,tl,opt,sched,dev,scaler,KD_TEMP,KD_ALPHA,use_mix=True)
        _,va_=eval_epoch(student,vl,crit,dev)
        ...
        if va_>best_acc:
            best_acc=va_; pat=0
            torch.save({"fold":TARGET_FOLD,"epoch":ep,"stage":2,"model_state_dict":student.state_dict(),"val_acc":va_},ckpt)
            log.info("  ★ best %.2f%%",100*va_)
        else:
            pat+=1
    ...
    student.load_state_dict(torch.load(ckpt,map_location=dev,weights_only=True)["model_state_dict"])
    swa_model=AveragedModel(student)
    swa_opt=AdamW(student.parameters(),lr=CFG["swa_lr"],weight_decay=CFG["weight_decay"])
    for swa_ep in range(1,CFG["swa_epochs"]+1):
        t0=time.time()
        trl_,_=train_epoch(student,tl,swa_opt,None,crit,dev,scaler,use_mix=True)
    ...
    del student,swa_model,swa_opt; torch.cuda.empty_cache()
