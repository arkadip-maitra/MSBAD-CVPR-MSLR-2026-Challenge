#!/usr/bin/env python3
"""
v38: ConvNeXtV2-Tiny @ 512×512 — resolution frontier.
Born-again chain: v11@224 → v18@384 → v25@448 → v38@512.
4th generation, pushing ConvNeXtV2 to the highest resolution yet.
"""
import os
...
OUT_DIR = Path("/data/slr/checkpoints_v38")
...
def main():
    ...
    for fold,(tri,vai) in enumerate(skf.split(keys,labels)):
        ...
        ba=V25_DIR/f"best_fold{fold}.pt"
        if ba.exists():
            load_born_again(model,ba,dev)
        else:
            ba2=V18_DIR/f"best_fold{fold}.pt"
