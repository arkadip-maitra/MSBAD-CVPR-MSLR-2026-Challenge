...
CNN v11: ConvNeXtV2-Tiny born-again from v10 + cross-architecture Knowledge Distillation (CAFormer teachers teach CNN)
"""
...
OUT_DIR = Path("/data/slr/checkpoints_v11")
...
TEACHER_CONFIGS = [
    {"dir": VIT_V8_DIR, "cls": MultiScaleCAFormer, "model_name": "caformer_s18.sail_in22k_ft_in1k",
     "kwargs": {"drop_path_rate": 0.0, "proj_dim": 384, "drop_rate": 0.3, "ms_samples": 5}},
    {"dir": V7_DIR, "cls": ConvNeXtGeM, "model_name": "convnext_tiny.fb_in22k_ft_in1k",
     "kwargs": {"drop_path_rate": 0.0}},
]

PL_CONFIGS = [
    {"dir": V10_DIR, "cls": MultiScaleConvNeXtV2, "model_name": "convnextv2_tiny.fcmae_ft_in22k_in1k",
    ...
    for cfg in configs:
        ...
        for fold in range(CFG["n_folds"]):
            ...
            if not ckpt_path.exists():
                continue
            m = cls(model_name, CFG["n_classes"], pretrained=False, **kwargs).to(dev)
            ckpt = torch.load(ckpt_path, map_location=dev, weights_only=True)
...
def main():
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(keys, labels)):
        ...
        born_again_path = V10_DIR / f"best_fold{fold}.pt"
        if born_again_path.exists():
            load_born_again(model, born_again_path, dev)
        else:
            log.info("  No v10 checkpoint for fold %d, using ImageNet init", fold)
    ...
    log.info("Loading v11 checkpoints for ensemble …")
    models_v11 = []
    for fold in range(CFG["n_folds"]):
    ...
    probs_no = predict_ensemble_tta(models_v11, eval_loader, dev, use_tta=False)
    ...
    probs_v11 = predict_ensemble_tta(models_v11, eval_loader, dev, use_tta=True)
    preds_v11 = probs_v11.argmax(dim=1).numpy()
    _write_submission(eval_keys, preds_v11, out / "submission_tta.csv")
    log.info("Submission (TTA) → %s", out / "submission_tta.csv")
    changed = (preds_v11 != preds_no).sum()
    log.info("TTA changed %d / %d (%.1f%%)", changed, len(preds_v11),
             100 * changed / len(preds_v11))

    torch.save({"keys": eval_keys, "probs": probs_v11},
               out / "eval_probs_tta.pt")
    log.info("Saved v11 TTA probs → %s", out / "eval_probs_tta.pt")
